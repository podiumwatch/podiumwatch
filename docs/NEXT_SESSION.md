...
The Podium Watch Fan Poll (cross country launch, both genders, divisions 1-4) is built and tested, but **not yet live in any database** -- `install/09_FAN_POLL.sql` is written and waiting for the user to review and run it in Supabase. Until it runs, no ballot can be cast. Full detail in `docs/SESSION_LOG.md` and `docs/DECISIONS.md`, both 2026-08-06. Separately, and already live: 422 `team_pages.cross_country_girls_division` values were populated from real official OHSAA PDFs the user supplied this session, closing most of the girls cross country data gap that shaped this feature's launch scope. Next steps once the migration is run: schedule and open the first real voting weeks through `/admin/fan-poll/`, live-verify a full ballot submission end to end, decide what to do about the 46 girls-only-program schools with no team page yet, build the actual results-email sending mechanism (only the opt-in capture exists so far), and turn on track and field once real division data exists for it (the schema is already ready -- no migration needed, just new weeks and a small `scripts/build.mjs` change).
