...
- **Root cause of the timing bug, found from the real race's own database rows**: `pollForRaceStart()`/`initialize()`'s recovery logic preferred a locally-cached (IndexedDB) wall-clock anchor over the fresh, correct one just fetched from the server. Any device that had ever locally recorded race-state for that exact session before the real start (a rehearsal, an earlier test-and-restart) kept using the stale anchor for the rest of the race, producing a real, confirmed, constant offset on its captures (+7.71s and +27.54s in the two real races checked). Fixed by always preferring the server's canonical `race_started_at` and self-healing the local cache.
- Helpers (race-day-code sessions) can no longer start, finish, restart, or adjust a race's clock -- enforced server-side (`RACE_DAY_CODE_ALLOWED_ACTIONS`), not just hidden in the UI. This is the same permission gap that caused the real bug above.
- Helper code shrank from 8-character alphanumeric to a 4-digit numeric code, with a new expiry (5 days) and a collision-retry loop (the smaller 10,000-value keyspace needs both).
- New shared `lib/todays_race_service.mjs` (`classifyRaceDay()`) picks the one relevant race right now (live > next today > most recently finished today) -- used by both the coach's new "Today's Split Watch" card on `/team-home/` and the helper join flow's smart auto-routing (no more landing on a historical meet list).
- Parent live link (`/race/?race=<id>`) gained a pre-race waiting state -- it already had the right privacy model (opt-in `spectator_visible`, safe field allow-list) from an earlier phase, it just had no UI for before the race starts.
- New coach-only Race Clock Adjustment feature -- aligns Split Watch with an official scoreboard without ever touching a raw capture timestamp, with an audit trail.
- A lightweight server-clock-offset correction for devices whose wall clock disagrees with the server, and a genuine atomic guard against two near-simultaneous Start Race calls both believing they won.
- `npm run build`/`run check`/full `npm test` all clean. Live-verified against real production Supabase with a fully disposable throwaway team/coach account (created, exercised, deleted, confirmed gone) -- covers the canonical-start fix, smart routing, every helper permission boundary, and the parent link's three states. Real mobile Playwright screenshots (iPhone 13/Pixel 7) confirm the new UI renders correctly.
- **Not yet done**: `install/24_SPLIT_WATCH_RACE_DAY_FIXES.sql` has not been applied to production (needs approval) -- until then, the 4-digit code's real generate/regenerate/revoke round trip and Race Clock Adjustment's actual database write are unverified live (the pure logic for both is unit tested and passing). Not pushed, not deployed. "Helper Connection Status" (a `TIMING CREW` list of currently-connected helpers with per-helper revoke) from the spec's Section 8 was deliberately deferred -- today a coach can only revoke the whole code (disconnects everyone), which is coarse but functional; flagged to the user as a known gap, not silently dropped.
