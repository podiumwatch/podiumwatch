...
Team logo/banner uploads are built and tested, but **not yet live in any database** -- `install/08_TEAM_MEDIA_UPLOADS.sql` (a new public `team-media` Storage bucket) is written and waiting for the user to review and run it in Supabase. Until it runs, uploading through `/team-editor/` will fail; pasting a URL directly still works as before. Full detail in `docs/SESSION_LOG.md` and `docs/DECISIONS.md`, both 2026-08-06. Next steps once the migration is run: upload a real file through the editor against a real claimed team, confirm it previews, confirm Save persists it, and confirm it renders on the public team page -- then decide whether unclaimed teams should get any photo path too (not built, since the literal request was read as upgrading the existing claimed-team editor).

A new team Instagram submission feature is also built and tested, but **not yet live in any database** -- `install/07_TEAM_INSTAGRAM_SUBMISSIONS.sql` is written and waiting for the user to review and run it in Supabase. Unlike everything else in this project, an accepted submission takes effect immediately (validated, rate limited, and fully reversible instead of admin-approved). Full detail in `docs/SESSION_LOG.md` and `docs/DECISIONS.md`, both 2026-08-06. Next steps once the migration is run: set `TEAM_INSTAGRAM_DIGEST_EMAIL` in Vercel, then live-verify the submit/revert/digest loop against a real team before this is pushed or deployed (the user has not yet reviewed the diff or approved a commit).
