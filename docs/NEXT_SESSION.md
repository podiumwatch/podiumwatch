...
Current priority: nothing outstanding for cross country. Next real steps whenever revisited: build the results-email sending mechanism (only the opt-in capture exists), and turn on track and field once real division data exists for it (schema is already ready).
