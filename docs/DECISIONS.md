...
## 2026 08 24 Eight 2026 preseason team power ranking articles + interactive components

### Decision

Published all 8 supplied 2026 preseason cross country team power-ranking articles (Boys/Girls x Divisions 1-4) as real `content/stories/*.md` stories, each carrying its 5 required interactive components (Race Board, How the Race Was Won, Fifth Runner Factor, Displacement Report, Reader Predictions), a sticky section nav, a Team Compare drawer, a Share This Team downloadable image card, and explanatory tooltips -- built entirely on the supplied, unaltered editorial copy and `podium-watch-2026-preseason-interactive-data.json` dataset, with a new real, Supabase-backed anonymous poll system for Reader Predictions.

### Reason

The user supplied complete, real editorial copy and a structured interactive-data JSON for all 8 classifications and asked for every interactive component to be genuinely built (not stubbed), the articles wired into the main feed, and the homepage/rankings-hub "power rankings" updated from this real data -- explicitly ruling out a localStorage-only poll as an acceptable final implementation.

### Key implementation decisions

1. **Content system: `content/stories/*.md`, not a new page type.** These are structurally season-preview stories, matching the existing "Top 25 Returning" precedent exactly (same frontmatter shape, `renderMarkdown()` pipeline) -- reusing it means the 8 articles get the main feed, `/stories/`, category pages, search index, sitemap, and RSS for free, with zero new plumbing. `slug` is set explicitly to the JSON's own `articleSlug` per classification so the story route and the poll system's `article_slug` join key are always the same value, never two independently-maintained copies.
2. **Component markers stay literal.** `[[PODIUM_WATCH_COMPONENT: RACE_BOARD]]` etc. are kept verbatim from the supplied article text rather than translated to raw HTML in the Markdown source (`src/lib/markdown.mjs` has no raw-HTML passthrough by design). A new single-line-marker case in `renderMarkdown()`, modeled directly on the existing `[[youtube:ID]]` case, emits a typed, fixed-id placeholder div (`id="race-board"`, `"race-build"`, `"fifth-runner"`, `"depth"`, `"reader-picks"`) that `public/scripts/preseason-article.js` hydrates client-side and the sticky nav links to -- the fixed id (rather than an auto-generated heading id) is what keeps the nav's anchors correct even if a future article's heading wording ever drifts.
3. **A new poll table, not an extension of Fan Poll (`install/09_FAN_POLL.sql`).** Fan Poll is a 16-team ranked ballot keyed by a hashed *email*, one per week. Reader Predictions is a single-option pick across up to 3 small fixed-choice questions per article, keyed by an anonymous hashed *browser token* (explicitly no email/name, per the spec), one vote per article/poll/token. The two are different on every axis; forcing Reader Predictions into Fan Poll's shape would mean collecting an email it has no reason to need. New migration `install/22_ARTICLE_POLLS.sql` (one table, `article_poll_votes`), new `lib/article_poll_service.mjs` (mirrors `fan_poll_service.mjs`'s pure-reducer-plus-Supabase-I/O shape), new `api/article-polls/{vote,results}.js` (mirrors `api/fan-poll/ballot.js`'s honeypot/HMAC-address-hash/error-shape conventions). Poll *definitions* (questions/options) are never duplicated into the database -- they stay the single JSON file's responsibility; the server validates every vote's poll/option id against that same file (`lib/preseason_data.mjs`) before it ever reaches the database, so a forged option id is rejected server-side, not just hidden client-side.
4. **`content/rankings/*.csv` was not used for these.** That system is athlete-performance-list shaped (`rank, athlete, school, ..., timeOrMark`) -- a team's simulated score has no column there, and inventing one would mean either corrupting that schema or fabricating fake per-athlete rows. Instead, `/rankings/cross-country/{gender}/division-{n}/` gained a real, prominent callout card (`preseasonClassificationFor()` in `scripts/build.mjs`) linking straight to the matching article -- this is "go under rankings cross country and then correct division boys or girls" without touching the CSV-driven system at all.
5. **Homepage Power Rankings widget now shows real data.** `powerRankingData` in `homePage()` was previously a mix of stale hardcoded teams (boys) and literal placeholder text ("Updated teams coming soon", girls D1-D3) -- both replaced by the top 4 *ranked* (never honorable-mention) teams per classification, read directly from the bundled JSON, with each row now linking straight to its own article instead of the generic `/rankings/cross-country/` landing page.
6. **Accent color is scoped per-page via a CSS custom property, not a new sitewide token.** `#2E7D32` (boys) / `#C2185B` (girls) come straight from the JSON's own `accent` field and are set inline as `--pw-accent` on the article's root element -- confirmed via WCAG contrast math (white-on-accent and accent-on-paper both clear 4.5:1 for both colors) before using either as real text/background color, rather than introducing a permanent `--pink` token into `src/styles/main.css`'s sitewide palette for a feature that only ever needs it on these 8 pages.
7. **Race Board reuses the sitewide `.table-scroll` responsive pattern** (a real `<table>` with `data-label` on every `<td>`, already collapsing to a stacked per-row "card" layout below 700px sitewide) rather than hand-building a separate mobile card component -- sort-by-column and tap-to-expand-roster are layered on top of that same already-tested markup in `public/scripts/preseason-article.js`, not a second DOM shape.

### Alternatives considered

1. A dedicated `src/pages/*.mjs` page per article (8 new files) instead of `content/stories/*.md` -- rejected; would have needed hand-built feed/search/sitemap/RSS wiring that the existing story pipeline already provides for free, for content that is structurally already a story.
2. Extending `fan_poll_weeks`/`fan_poll_ballots` for Reader Predictions -- rejected per point 3 above; different identity model, different ballot shape, and Fan Poll's own schema comment already documents why it is intentionally not general-purpose.

### Files or systems affected

New: `content/stories/20260824_preseason_{boys,girls}_d{1-4}_top20.md` (8), `public/data/podium-watch-2026-preseason-interactive-data.json`, `install/22_ARTICLE_POLLS.sql`, `lib/preseason_data.mjs`, `lib/article_poll_service.mjs`, `api/article-polls/vote.js`, `api/article-polls/results.js`, `public/scripts/preseason-article.js`, `scripts/test-article-polls.mjs`. Modified: `src/lib/markdown.mjs` (component-marker case), `scripts/build.mjs` (`preseasonClassificationFor`/`preseasonArticleHref`/`preseasonPowerRows` helpers; `homePage()`'s `powerRankingData`; `divisionPage()`'s rankings-hub callout; `storyPage()`'s opt-in interactive-article rendering), `src/styles/main.css` (new `.pw-*` component styles), `package.json` (`test:article-polls` added to `npm test`).

### Follow up

`npm run build`/`run check`/full `npm test` all clean (292 pages, zero link/asset problems). A full Playwright pass against the real built site confirmed: all 8 routes load their correct classification data; 20 ranked + 5 honorable-mention teams; roster tap-to-expand shows real supplied athlete names/times; sorting; the boys-D1 tiebreak badge cites the exact supplied score/teams/places; score-progression legend totals match Race Board scores for the top 5 (also cross-checked programmatically for all 8 classifications in `scripts/test-article-polls.mjs`); Team Compare drawer and Share-This-Team download both work with real data; boys pages carry the real `#2E7D32` accent and girls pages the real `#C2185B` accent; the rankings-hub callout and homepage Power Rankings widget both now show real article data instead of placeholders. **Not yet done:** `install/22_ARTICLE_POLLS.sql` has not been run against Supabase (needs explicit go-ahead per standing practice) -- until then, Reader Predictions correctly degrades to "vote below, live totals will appear after you do" rather than erroring, but a real vote round trip (duplicate voting, invalid options, cross-article isolation) is still unverified against a live database. Not pushed or deployed.

## 2026 08 26 Split Watch: Live Race Day Fixes (real race-day feedback, Shelby County Preview 2026-08-25)

### Decision

Diagnosed and fixed the real ~10-second timing discrepancy from Split Watch's actual first live race, then rebuilt the surrounding race-day workflow (canonical race start, coach's race-day entry point, helper routing, a 4-digit helper code, the parent live link's pre-race state, and a new Race Clock Adjustment feature) based on the user's detailed race-day feedback spec. Everything here is additive/behavioral -- no existing Split Watch table was altered destructively, and nothing has been pushed, deployed, or migrated in production without approval.

### The timing bug -- root cause, with direct evidence

Queried the real `race_splits` rows from the real meet (not a re-read of old code): one race showed a constant +7.71s offset on every Finish-checkpoint split from one specific device; a second race that same day showed a constant +27.54s offset, also from one device, also only after that device's Mile-1 splits (which were correct). A *constant*, non-drifting offset is the signature of a device timing against the wrong start reference, not clock drift. The user confirmed neither helper device ever pressed Start Race -- both sat on the auto-transitioning pre-race screen the whole time -- which pointed at `pollForRaceStart()`/`initialize()`'s recovery logic, not a second Start press:

```js
const anchorMs = localState?.race_started_at_wall_clock_ms || Date.parse(fresh.session.race_started_at);
```

This preferred a **locally cached** wall-clock anchor (IndexedDB, keyed by race_session_id) over the **fresh value the device just fetched from the server**, and nothing besides tapping Restart on that *same* device ever cleared that cache. Any device that had ever locally recorded a race-state for that exact session before the real start (a rehearsal tap, an earlier test-and-restart cycle -- normal for "prepare the night before") would silently keep using the stale anchor for the rest of the race. Fixed by inverting the priority everywhere this pattern appeared (`pollForRaceStart()`, `initialize()`'s live-recovery branch, and the Start Race button handler): the server's own `race_started_at` always wins, and the local cache is immediately overwritten to match it (self-healing) rather than trusted first.

### Key implementation decisions

1. **`startRace()` now tells the truth about who started the race.** It returns `{ session, started_by_this_request }`, decided by a single atomic guarded UPDATE (`.neq("status", "live")`) rather than a read-then-write -- closes the real race condition where two near-simultaneous Start Race calls could otherwise both believe they won. The client no longer blindly trusts its own locally-captured anchor; a losing call now recovers from the server's real `race_started_at`, exactly like joining an already-live race.
2. **A lightweight server clock offset, not a new sync architecture.** `sync.js`/`sessions.js` responses now carry `server_now`; the client brackets the request/response round trip and calls `Timer.setClockOffsetMs()`, which only ever affects the lower-precision *recovered* path (`race-timer.js`) -- the live monotonic path never needed wall-clock agreement and is untouched. One-sample-per-request, refreshed on every sync; no NTP-style averaging, matching the "don't add complexity the architecture doesn't need" instruction.
3. **Helpers can no longer start, finish, restart, or adjust a race's clock -- at the permission layer, not just the UI.** `RACE_DAY_CODE_ALLOWED_ACTIONS.sessions` dropped `start_race`/`finish_race`/`restart_race` entirely (down to `list`/`detail`/`today`); `adjust_clock` was never added to the helper's `sync` set. The client mirrors this (`viewer.type` returned on every response) by hiding those controls for a helper session, but the server-side refusal is what actually matters -- this is the direct fix for Problem 4, and it is the same permission gap that produced the real timing bug above.
4. **4-digit numeric helper code, same underlying system.** `team_race_day_codes`/`race_day_sessions` (install/19) were reused, not duplicated -- `generateRaceDayCode()` now produces 4 digits instead of 8 alphanumeric characters. A 10,000-value keyspace can't rely on "collision is astronomically unlikely" the way the old 32^8 space could, so `regenerateRaceDayCode()` gained a bounded collision-retry loop (`codeHashIsTaken()`, checked against other teams' *active, unexpired* codes only) and a new `expires_at` column (5-day validity -- long enough to prepare the night before, short enough that the shared keyspace never carries much history). An expired code's value becomes reusable automatically, no cleanup step needed.
5. **A new shared "what race matters right now" module (`lib/todays_race_service.mjs`), not four separate ad hoc queries.** `classifyRaceDay()` is a pure function (unit tested directly): exactly one live race always wins outright; zero live but something upcoming today routes to that; nothing live or upcoming but something finished today routes to that (so a helper never lands on historical noise after a race ends); more than one simultaneously-live race is the one case that surfaces a short choice instead of auto-routing. Also fixed a latent, unrelated bug this surfaced: `team_workspace_service.mjs`'s own "today" computation was raw `new Date().toISOString().slice(0,10)` (UTC), which silently rolls to the wrong calendar day for several hours every evening for an Ohio-only product -- replaced with a real America/New_York date via `Intl.DateTimeFormat`.
6. **Race Clock Adjustment never touches a raw capture timestamp.** `race_sessions` gained `race_start_adjustment_seconds` (signed, default 0) plus an audit stamp (`race_clock_adjusted_at`/`race_clock_adjustment_note`). The coach enters the *current correct official elapsed time*; the server derives the needed correction from the untouched `race_started_at`, applies the same delta to every already-recorded split's `elapsed_seconds` (never to `wall_clock_captured_at`), and the client applies the same correction to its own live display and to every future capture. A second adjustment later in the same race still corrects the whole timeline consistently (delta from the *previous* stored correction, not a fresh absolute value stacked on top).
7. **`scheduled_start_time` is a plain SQL `time` column (install/11), not a timestamp -- found the hard way** (a full ISO string sent to it fails Postgres's `time` parser outright, confirmed against real production). The coach-facing field is a plain `<input type="time">`, paired with the race's own `race_date` wherever it's displayed (Plan page, parent waiting screen) rather than parsed alone as a full instant.
8. **The parent link (`/race/?race=<id>`) already had the right privacy model** (`race_sessions.spectator_visible`, opt-in, off by default, a hand-picked field allow-list with no goals/notes/device data) from an earlier phase -- it just had no pre-race UI. Added a waiting state (shown for `draft`/`scheduled` status) with the exact requested copy ("Live timing has not started yet... Keep this page open") and the scheduled time when set; the existing poller (`race-poll.js`, already backing off on error and pausing when the tab is hidden) is what makes it flip to live with no new URL and no manual refresh -- no new realtime architecture was needed.
9. **Helper code and parent link copy now explicitly distinguish the two** ("this code is not" a read-only view) everywhere the old code's sharing copy previously blurred them together, per the explicit instruction not to combine the two systems just because both involve sharing.

### Alternatives considered

1. Reaching into other devices' IndexedDB to clear a stale cache on regenerate/restart -- impossible (no channel to another device), and unnecessary once the priority inversion above makes a stale local cache self-correct on the very next server round trip.
2. A full NTP-style multi-sample clock sync -- rejected as more complexity than a single-meet, short-lived race clock needs; a single bracketed round-trip sample refreshed every sync cycle is adequate and matches the existing architecture's own restraint.
3. A brand-new table for the 4-digit code system -- rejected; the existing `team_race_day_codes`/`race_day_sessions` tables and the whole `requireSplitWatchAccess`/`RACE_DAY_CODE_ALLOWED_ACTIONS` permission-scoping mechanism already did everything needed except expiry + collision handling, both added additively.

### Files or systems affected

New: `install/24_SPLIT_WATCH_RACE_DAY_FIXES.sql`, `lib/todays_race_service.mjs`. Modified: `lib/race_day_auth.mjs` (4-digit codes, expiry, collision retry, narrowed `RACE_DAY_CODE_ALLOWED_ACTIONS`), `lib/split_watch_service.mjs` (`started_by_this_request`, atomic start guard, `adjustRaceClock()`, `scheduled_start_time` as time-of-day), `lib/team_workspace_service.mjs` (real Eastern "today", `todaysRaceDay`), `api/split-watch/sessions.js` (`today` action, `viewer` field), `api/split-watch/sync.js` (`adjust_clock` action, `server_now`/`viewer` fields), `public/scripts/race-timer.js` (`setClockOffsetMs`), `public/scripts/split-watch-live.js` (canonical-start handling, viewer-gated UI, clock-offset wiring, Adjust Race Clock dialog), `public/scripts/split-watch-races.js` (smart auto-routing), `public/scripts/split-watch-plan.js` (parent link + spectator toggle + scheduled time UI), `public/scripts/race-public.js` (pre-race waiting state), `public/scripts/split-watch-join.js`/`team-home.js`/`split-watch-hub.js` (4-digit UI, Today's Split Watch card, code-sharing copy), `src/pages/{splitwatchlive,splitwatchplan,splitwatchjoin,teamhome,splitwatch,racepublic}.mjs` (matching markup), `scripts/test-split-watch.mjs` + `scripts/test-race-day-access.mjs` (new/updated tests, including the previously-untested `RACE_DAY_CODE_ALLOWED_ACTIONS` boundary).

### Follow up

`npm run build`/`run check`/full `npm test` all clean. Live-verified against real production Supabase using a fully disposable throwaway team/coach account/races (created, exercised, and deleted, confirmed gone) -- the canonical-start atomic guard, the stale-anchor fix, smart routing (coach and helper), the parent link's pre-race/live/finished states, and every helper permission boundary (403 on start/finish/restart/adjust, still allowed to push real splits) all confirmed directly against the real database, not a mock. Real mobile-viewport Playwright screenshots (iPhone 13 / Pixel 7) confirmed the Today's Split Watch card, the helper's controls-hidden live screen, and the parent waiting screen all render correctly with no horizontal overflow. **Not yet verified** (blocked on the migration below): the 4-digit code's real generate/regenerate/revoke round trip and Race Clock Adjustment's actual database write both need `install/24`'s new columns, which have not been applied to production. **Not pushed, not deployed, no migration applied** -- awaiting review and explicit approval.

## 2026 08 28 My Podium (Projects 0-4, plus the homepage connection, Project 9)

### Decision

Built a mobile-first navigation rebuild plus a new personalized, accountless destination, `/my-podium/`, against a spec of Projects 0-9. Prework re-confirmed (a fourth time this project) that the previously-reported "missing `src/pages/adminteams.mjs`" build failure does not exist in this checkout -- the file is present, tracked, and correctly imported; no placeholder was created or needed.

### Reason

The spec explicitly required inspecting the real repository before deciding architecture, reusing existing data services rather than duplicating them, and never fabricating data (ranking movement, live status, relationships) the underlying tables don't actually support. Concretely:
1. **My Podium's dashboard is entirely client-rendered**, never build-time -- this is a static site generator with no live database access at build time; a build-time "next meet" would go stale the instant the calendar advanced.
2. **Preferences live only in `localStorage`** (`podiumWatch.myPodium.v1`, `public/scripts/my-podium-store.js`), versioned, with a defensive read path (corrupted JSON, wrong schema version, storage unavailable all fall back to a safe empty shape) rather than a new account system -- explicitly out of scope for this pass per the spec's own privacy-sensitivity caution.
3. **Division is never asked, it's derived.** `team_pages` carries real `cross_country_boys_division`/`cross_country_girls_division`/`track_boys_division`/`track_girls_division` columns -- once a visitor picks a team + sport + gender, the division shown is read directly from that team's own verified row, which is both more accurate and less code than a manual picker.
4. **Ranking movement is never shown.** `previousRank` exists as a real CSV column but is empty on every published row today (this is the site's first, preseason-only ranking cycle) -- My Podium shows current rank + date only, honestly.
5. **"Live" is never shown from a date match alone.** No verified sitewide "is this team's race live right now" signal exists (only a real per-session one); My Podium's Race Day states are `today`/`scheduled`/`results_pending`/`results_available` only.
6. **Card order deliberately leads with Personal Context, then Poll and Rankings**, ahead of Next Meet/Latest Result -- this diverges from the originating spec's suggested order, matching the user's own explicit, standing feedback earlier the same session about what the homepage (and, by the same logic, My Podium) should lead with.

### A real, incidentally-discovered production bug, fixed as part of the audit

`POST /api/athletes/` was returning HTTP 500 for every visitor in production, confirmed live and reproduced locally against real Supabase data. Root cause: two `.in("profile_id", profileIds)` queries in `api/athletes/index.js` sent the full 846-id athlete list as one PostgREST query-string parameter, well past the point (roughly 300-400 ids, confirmed empirically) where the request URL exceeds the server's length limit, returning a bare "Bad Request." This silently broke the public `/athletes/` page for every visitor, unrelated to anything in this session. Fixed by batching both queries into chunks of 150 ids (`selectInBatches()`), merging and re-sorting client-side so per-profile "first entry wins" behavior is unchanged. Verified against real production data: 846 profiles, HTTP 200.

### Alternatives considered

1. A manual division picker in onboarding (matching the original spec's suggested flow) -- rejected once `team_pages`'s real division columns were confirmed; asking a visitor to self-report something already verified elsewhere risks disagreement with the truth for no benefit.
2. Fetching stories client-side for the Stories card -- rejected; stories are legitimately build-time-static content (same as rankings), so the relevant story list is embedded as a small JSON payload at build time instead (`storiesDataScript()` in `src/pages/mypodium.mjs`), avoiding a new fetch path or a second story dataset.
3. Leading the dashboard with Next Meet/Latest Result (the spec's suggested default order) -- rejected in favor of Personal Context, then Poll, then Rankings, then meet/result cards, matching the user's own stated preference from the same session's homepage work.

### Files or systems affected

New: `src/pages/mypodium.mjs`, `public/scripts/my-podium-store.js`, `public/scripts/my-podium-data.js`, `public/scripts/my-podium.js`, `docs/MY_PODIUM_MASTER_BUILD_PLAN.md`, `docs/MY_PODIUM_DATA_MAP.md`, `docs/MY_PODIUM_TEST_MATRIX.md`. Modified: `api/athletes/index.js` (the batching fix above), `src/lib/html.mjs` (5-item mobile dock, `podium` icon), `src/config/site.mjs` (new nav entry), `src/styles/main.css` (new design tokens), `public/scripts/homepage.js` (Follow Your School panel migrated onto the shared preference store, dismissible new-visitor promo, returning-visitor preview), `scripts/build.mjs` (route registration).

### Follow up

`npm run build`/`run check`/full `npm test` all clean. Verified with roughly 50 Playwright checks against a local harness serving the real `api/*.js` handlers with real production Supabase data (onboarding end-to-end with real team/athlete search, refresh persistence, corrupted/unavailable storage recovery, edit/clear with confirmation, explore-without-setup mode, the dismissible homepage promo, the returning-visitor preview, real positive-data Next Meet rendering, `meetStatus()` never returning `"live"`, no horizontal overflow at 360/390/430px), then re-verified against the live production site post-deploy. Pushed and deployed; commit `6ff31f4`. This DECISIONS.md entry and the matching SESSION_LOG.md entry were written retroactively during the Project 5 follow-up work below -- a real documentation gap this audit found and is closing in the same pass.

## 2026 08 28 My Podium Project 5 -- email alerts + account sync

### Decision

Corrected an earlier assumption (the original Project 5 roadmap entry claimed "authentication architecture... none of this exists today") after a real repository audit, then built two safe, well-scoped slices on top of infrastructure that turned out to already be real and mature: wiring My Podium into the existing email alert system, and a new lightweight self-serve account for cross-device preference sync.

### Reason

Three parallel Explore passes plus a Plan pass established the real picture before any code was written:
1. **Coach and photographer accounts are already fully open self-serve** Supabase Auth signups (`lib/team_auth.mjs`, `lib/photographer_auth.mjs`) -- no invite needed, since they only ever grant access to the signer-upper's own thing. Athlete/guardian accounts are the opposite -- coach-invite-only, gated by a single self-attestation consent checkbox, no real identity verification, no COPPA logic anywhere in the codebase (confirmed by an exhaustive grep).
2. **A real, mature, double-opt-in email alert system already existed and was live**: `team_followers`/`team_follows`, `lib/engagement_service.mjs`, `api/followers/*`, the public `/follow/` page -- requires only an email, no Supabase Auth account. Confirmed directly against production: `engagement_settings.public_following_enabled = true`, `notification_mode = "live"`. But `RESEND_API_KEY`/`RESEND_FROM_EMAIL` are not configured in production, so no email feature on the whole site can actually send right now -- a real external credential blocker, independent of any code.
3. **`docs/RECRUITING_PHASE_THREE_ARCHITECTURE.md` already established** that anything touching a specific minor's identity (a self-service athlete profile claim) must always go through human admin review, no exceptions -- this plan does not revisit that, and explicitly does not touch it.

Given this, a My Podium account is modeled on the open team/photographer pattern, not the invite-only athlete/guardian one, because it only ever grants a signed-in user access to their own preferences -- never another person's data, never any athlete's private information.

### Key implementation decisions

1. **Slice A adds zero new tables and zero new `engagement_service.mjs` logic.** My Podium's context card gained a "Get email alerts for [school]" action that calls the existing `POST /api/followers/subscribe` exactly as-is, including its existing default `source: "team_profile"` -- a new `source` value was deliberately not introduced, since `team_followers`/`team_follows` predate this repo's tracked `install/` migrations and their exact constraints couldn't be safely verified without a live write test; reusing the known-safe default costs nothing functionally. The email a visitor enters is never written to `localStorage` -- only a bare, non-PII `alertsRequestedAt` timestamp is kept (`markAlertsRequested()` in `my-podium-store.js`), so the capture form doesn't keep re-prompting a visitor who already asked.
2. **Slice B's new table (`my_podium_accounts`) has no RLS policies**, matching this project's one universal pattern confirmed against `install/14_PHOTOGRAPHER_NETWORK.sql`: enable row level security, revoke all from `anon`/`authenticated`, grant all to `service_role`. Authorization happens entirely in application code via `lib/my_podium_auth.mjs` (a verbatim mirror of `lib/team_auth.mjs`'s shape) -- this project has never used client-direct-to-Supabase RLS policies and this table does not start.
3. **`preferences` is one opaque `jsonb` document**, a server-side mirror of exactly what `my-podium-store.js`'s `getPreferences()` already produces locally, not a new relational schema to keep in sync with the client's own shape.
4. **Reconcile-on-load, not a background sync daemon.** `public/scripts/my-podium-sync.js`'s `reconcile()` runs once at `/my-podium/` boot: a cheap local-only `getSession()` check first (no network cost for the overwhelming majority of accountless visitors), then, only if signed in, a real comparison against `GET /api/my-podium/me` -- no server row and local data present pushes up; a server row with no local data (a new device) pulls down; both present compares timestamps, last-write-wins, with the server as the final word on conflicts (`api/my-podium/sync.js` returns the stored, winning state rather than overwriting when the incoming push is stale).
5. **"Clear all" while signed in clears the synced copy first**, synchronously, before the local clear and page reload -- found and fixed during verification: without this, a debounced background push can't complete before the immediate page reload, so the old synced preferences would simply reappear on the very next reconcile. `api/my-podium/clear.js` resets the row rather than deleting it; no account-deletion precedent exists anywhere in this codebase yet, for any tier -- explicitly out of scope here, flagged as a real, pre-existing, cross-cutting gap rather than solved ad hoc.

### Alternatives considered

1. Introducing a new `source: "my_podium"` value on `team_followers` (would need a small migration widening a possibly-existing check constraint) -- rejected in favor of reusing the existing default, avoiding any schema risk for a small attribution-only gain.
2. A background polling sync loop -- rejected as unnecessary complexity; reconcile-on-load plus push-on-change (debounced via the store's existing `onChange()` pub/sub) covers every real scenario (new device, cross-device edit, sign-out) without a persistent timer.
3. Full account deletion in this pass -- rejected; no precedent exists for any of this project's four-plus account tiers, and a real "delete my account" flow should be designed once, site-wide, not bolted onto My Podium alone.

### Files or systems affected

New: `install/31_MY_PODIUM_ACCOUNTS.sql`, `lib/my_podium_auth.mjs`, `api/my-podium/me.js`, `api/my-podium/sync.js`, `api/my-podium/clear.js`, `src/pages/mypodiumlogin.mjs`, `public/scripts/my-podium-auth.js`, `public/scripts/my-podium-sync.js`. Modified: `public/scripts/my-podium-store.js` (`markAlertsRequested()`, `applyRemotePreferences()`), `public/scripts/my-podium.js` (alerts CTA/form, sync boot sequence, sign-out wiring, the Clear-all fix above), `src/pages/mypodium.mjs` (sync status markup, new script tags), `scripts/build.mjs`/`src/lib/html.mjs` (route registration, private-route lists), `docs/MY_PODIUM_MASTER_BUILD_PLAN.md`/`docs/MY_PODIUM_DATA_MAP.md` (Project 5 moved from roadmap to implemented).

### Follow up

`npm run build`/`run check`/full `npm test` all clean. Slice A verified end to end against real production Supabase (a real `team_followers`/`team_follows`/`team_notification_deliveries` row was created by a real throwaway submission, confirmed the exact email-delivery-not-configured failure, then deleted and confirmed gone). Slice B verified with a real, disposable, admin-created test account (email pre-confirmed server-side, no real email ever sent) across two simulated browser-context "devices" against real production Supabase: first sync (push), a brand-new device pulling synced preferences down automatically instead of showing onboarding, a cross-device edit propagating on reload, "Clear all" while signed in correctly preventing a later reconcile from resurrecting the cleared data, and sign-out leaving local preferences fully untouched -- 11 of 11 checks passed. All test data (the auth user, its `my_podium_accounts` row, and the follower/follow/delivery rows) deleted after and confirmed gone via a direct re-query. Migration `install/31_MY_PODIUM_ACCOUNTS.sql` was run by the user directly in the Supabase SQL Editor (copied to clipboard, per standing practice) after explicit approval, then verified with a live select before any Slice B testing proceeded. **Not yet fixed** (outside this session's control): `RESEND_API_KEY`/`RESEND_FROM_EMAIL` remain unconfigured in production, so Slice A's email alerts are wired and correct but not yet actually delivering mail.

## 2026 08 30 Athlete Profiles and Team Hubs: connect, fix, and extend the identity system

### Decision

Given a long, prescriptive spec for a connected athlete identity/results/team/roster system, a repository audit (three parallel Explore agents plus a Plan agent, all read-only) found almost everything requested already existed in the schema, just disconnected or unused, and shaped the plan into a "connect, fix, and extend" pass rather than a rebuild -- exactly two small additive migrations, everything else application-layer wiring. A handful of scope decisions were made along the way, each resolved by checking real data rather than assumption:

1. **Season leaders reuse the existing all-time-PR view (`athlete_best_performances`), not a new season-specific aggregate.** No query for "this athlete's best mark set specifically within this season" exists anywhere in the codebase; building one was out of scope for this pass. The rendered section's own copy says exactly this ("an all-time personal record, not necessarily set during this specific season") rather than implying something more precise than what's actually shown.
2. **"Program level" was dropped as a roster filter after checking the real schema, not built on the plan's unverified assumption.** `program_level` (high_school/middle_school/club) is a school-level field on `ohio_schools`, confirmed directly in `install/01`, not a per-athlete roster attribute -- there was nothing there to filter a roster by. A grade filter was built instead, using real per-entry data.
3. **External source identity mapping (Athletic.net/MileSplit ids) reuses `athlete_profile_aliases` with a new `alias_type` column, rather than a new table.** One table now serves both "also known by this name" and "externally identified as" rows; only `alias_type = 'name'` rows ever feed search or import matching, keeping the "never match on name alone" rule intact.
4. **Unmerge only snapshots and restores the source profile's own pre-merge state, not the target's.** The existing merge RPC already mutates the target with `OR`'d booleans, unioned `primary_events`, and merged `metadata` -- capturing the target's full pre-merge state too would double the snapshot's scope for a case (undoing damage done *to* an already-correct target) that wasn't the actual problem being solved (recovering an accidentally-merged-away source). Stated as a known, honest limitation in the migration's own header comment and the admin UI's copy, not hidden. A merge performed before this migration shipped has no snapshot and can never be undone through the new mechanism, for the same reason.
5. **The pilot's specific "August 25, 2026 two mile result" was never found** anywhere in the repo or the user's Downloads folder after an exhaustive search across this and an earlier session. Flagged to the user directly rather than fabricated silently; the user approved a clearly-labeled, disposable, fabricated placeholder for verification purposes only, deleted immediately after use and never presented as real.

### Alternatives considered

1. A parallel merge-audit table capturing full before/after snapshots for both profiles in every merge -- rejected as unnecessary scope; the actual stated need was reversing an accidental merge (recovering the source), not auditing every field change on the target.
2. Making sitewide search (`search.mjs`) alias-aware too -- rejected for this pass; it is fully build-time-static from a bundled seed with zero connection to live `athlete_profiles`, and making it live is a separate architecture change. The alias fix lands on the live `/athletes/` directory and its API instead, which is where the pilot's "search resolves either spelling" requirement is actually verified.
3. A third import/ingestion pipeline for alias-aware matching -- rejected; extended the existing admin-UI-connected pipeline (`lib/recruiting_service.mjs`) instead of building a parallel path, only touching the separate crawler/staging pipeline for the shared grade-derivation fix both already needed.

### Files or systems affected

New: `install/32_ATHLETE_PROFILE_ALIASES_EXTEND.sql`, `install/33_ATHLETE_MERGE_REVERSAL.sql`. Modified: `lib/athlete_foundation_service.mjs` (`deriveGraduationYearFromGrade()`, `athleteVerificationStatus()`), `lib/recruiting_service.mjs` (alias-aware matching, resolve-and-learn, grade fix), `lib/result_ingestion_engine.mjs` (grade fix), `lib/team_roster_service.mjs` (grade fix, `syncTeamRosterProfiles()` wiring, `sync_profiles` action), `api/admin/athletes.js` (`save_alias`/`delete_alias`/`unmerge_profile`, alias-aware search, recent-merges list), `api/athletes/index.js` (alias-aware directory search), `api/athletes/detail.js` (shared verification-status function), `api/teams/roster.js` (`performance_leaders`, the season-sport mapping fix), `api/admin/recruiting.js` (thread `sport` param), `src/pages/adminathletes.mjs`, `src/pages/athletedetail.mjs`, `src/pages/teamprofile.mjs`, `src/pages/teamroster.mjs`, `src/pages/adminrecruiting.mjs`, and their corresponding `public/scripts/*.js` client files.

### Follow up

`npm run build`/`run check`/full `npm test` all clean after every rollout step. Live-verified against real production Supabase with a disposable local harness (real `api/*.js` handlers invoked in-process) and real, disposable, clearly-labeled test data, deleted and reconfirmed gone every time: the Philpot/Phlipot alias pilot, a full merge-then-unmerge round trip, ambiguous-match/resolve-and-learn/duplicate-prevention in the import pipeline, the roster-to-profile sync fix, season-leaders computation (only correct after the sport-mapping bug below was fixed), and a full Playwright pass at 360/390/430/1440px on both Russia's real live team page and a disposable athlete profile confirming zero horizontal overflow. **A real bug was found live, not before**: `team_seasons.sport` stores a display string ("Cross Country") while `athlete_performances.sport` uses a normalized enum (`cross_country`) -- the season-leaders query's first draft compared them directly and silently returned zero leaders every time; fixed with a small mapping function once caught. Not committed or pushed; `install/32`/`install/33` were run by the user directly in the Supabase SQL Editor after explicit approval and are confirmed live.
