// Mirrors public/scripts/athlete-login.js exactly, retargeted to the
// guardian tier (Team Workspace Phase Three). Reuses
// public/scripts/team-auth-client.js (window.PodiumTeamAuth) as-is, same
// as the athlete tier -- it's a generic Supabase session-management
// wrapper with nothing tier-specific in it.
(() => {
  const loadingBox = document.querySelector("[data-guardian-auth-loading]");
  const authShell = document.querySelector("[data-guardian-auth-shell]");
  const messageBox = document.querySelector("[data-guardian-auth-message]");
  const inviteBanner = document.querySelector("[data-guardian-invite-banner]");
  const authTabs = document.querySelector("[data-guardian-auth-tabs]");
  const signupTab = document.querySelector("[data-guardian-signup-tab]");
  const panels = Array.from(document.querySelectorAll("[data-auth-panel]"));
  const panelButtons = Array.from(document.querySelectorAll("[data-show-auth-panel]"));
  const signInForm = document.querySelector("[data-guardian-signin-form]");
  const signUpForm = document.querySelector("[data-guardian-signup-form]");
  const resetForm = document.querySelector("[data-guardian-reset-form]");
  const updatePasswordForm = document.querySelector("[data-guardian-update-password-form]");
  ...
  function redirectToGuardianHome() {
    window.location.replace("/guardian-home/");
  }
  ...
  async function redeemInviteIfPresent() {
    ...
    await apiFetch("/api/guardian/invite/", { action: "redeem", token: inviteToken }, accessToken);
  }
  ...
  signUpForm.addEventListener("submit", async (event) => {
    ...
    try {
      const { data, error } = await client.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: window.location.origin + "/guardian-login/?confirmed=1&invite=" + encodeURIComponent(inviteToken),
  ...
  resetForm.addEventListener("submit", async (event) => {
    ...
    try {
      const { error } = await client.auth.resetPasswordForEmail(email, {
        redirectTo: window.location.origin + "/guardian-login/?mode=update-password"
      });
      if (error) throw error;
      resetForm.reset();
  ...
  async function initialize() {
    try {
      ...
      if (inviteToken) {
        try {
          inviteInfo = await apiFetch("/api/guardian/invite/", { action: "validate_token", token: inviteToken });
      ...
      loadingBox.innerHTML =
        "<h2>Guardian accounts unavailable</h2><p>" +
