// Mirrors public/scripts/team-auth.js exactly, retargeted to the
// Photographer Network. Reuses team-auth-client.js (window.PodiumTeamAuth)
// as-is -- a generic Supabase session-management wrapper with nothing
// team-specific in it.
(() => {
  const loadingBox = document.querySelector("[data-photog-auth-loading]");
  const authShell = document.querySelector("[data-photog-auth-shell]");
  const messageBox = document.querySelector("[data-photog-auth-message]");
  const authTabs = document.querySelector("[data-photog-auth-tabs]");
  const panels = Array.from(document.querySelectorAll("[data-auth-panel]"));
  const panelButtons = Array.from(document.querySelectorAll("[data-show-auth-panel]"));
  const signInForm = document.querySelector("[data-photog-signin-form]");
  const signUpForm = document.querySelector("[data-photog-signup-form]");
  const resetForm = document.querySelector("[data-photog-reset-form]");
  const updatePasswordForm = document.querySelector("[data-photog-update-password-form]");
  ...
  function redirectToDashboard() {
    window.location.replace("/photographer-dashboard/");
  }
  ...
  signUpForm.addEventListener("submit", async (event) => {
    ...
    try {
      const { data, error } = await client.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: window.location.origin + "/photographer-login/?confirmed=1",
  ...
  resetForm.addEventListener("submit", async (event) => {
    ...
    try {
      const { error } = await client.auth.resetPasswordForEmail(email, {
        redirectTo: window.location.origin + "/photographer-login/?mode=update-password"
      });
      if (error) throw error;
      resetForm.reset();
  ...
  async function initialize() {
    try {
      ...
      loadingBox.innerHTML =
        "<h2>Photographer accounts unavailable</h2><p>" +
