...
export async function getDashboard(days = 30) {
  ...
  // Writer Portal's own review queue (lib/writer_portal_service.mjs)
  // previously had no sidebar badge at all -- a real gap, since it's one
  // of the tools staff actually check daily. A single targeted count
  // query here (not the heavier getPortalStats(), which runs several
  // queries to build a full stats page) matches this file's own
  // established per-subsystem convention.
  const submittedArticlesResult = await runSubsystem({
    key: "writer_portal_submitted",
    label: "Writer Portal submitted articles",
    issues,
    query: () =>
      supabaseAdmin
  ...
  if (submittedArticlesResult.available) {
    const submittedCount = Number(submittedArticlesResult.count || 0);

    if (submittedCount > 0) {
      addTask(tasks, {
        priority: "important",
        title: "Review submitted Writer Portal articles",
        detail: `${submittedCount} article${submittedCount === 1 ? " is" : "s are"} submitted and waiting for review.`,
        href: "/writer-portal/admin/review/",
        label: "Open Review Queue",
        count: submittedCount,
        category: "writer_portal"
      });
    }
  }
