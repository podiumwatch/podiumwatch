...
// Strips two real, confirmed-live ways ordinary pasted text (from a PDF,
// Word, or a mangled copy/paste elsewhere) can silently poison a save
// from then on: a NUL byte, which Postgres refuses to store in any
// text/json column at all (22P05 "unsupported Unicode escape sequence"),
// and a lone/unpaired UTF-16 surrogate, which PostgREST rejects outright
// as invalid JSON (PGRST102) before it ever reaches Postgres. Both throw
// a raw, unhandled error from whatever save call included them --
// confirmed 2026-09-10 against a real intern's draft (Natalie Stokes,
// "Wooster Invite") whose every autosave attempt failed silently this
// way once real pasted content was involved. Walks by UTF-16 code unit
// so a real surrogate PAIR (an actual emoji, valid) is left untouched --
// only a high surrogate with no following low surrogate (or vice versa)
// is dropped.
...
export async function createOrLinkFinishTimingAthleteProfile({
  ...
  const payload = {
    ...
    // "source_linked" is a real, valid status on athlete_performances,
    // but athlete_profiles' own check constraint doesn't include it at
    // all -- confirmed live 2026-08-31 when publishing a batch of
    // profiles created this way failed outright ("violates check
    // constraint athlete_profiles_verification_status_check"), a bug
    // that had been sitting dormant here the whole time since Finish
    // Timing's own auto_publish_enabled has never been turned on in
    // production. "editorial_source_linked" is the real equivalent for
    // this table, already used and displayed elsewhere in the admin/
    // public athlete UI.
    verification_status: publish ? "editorial_source_linked" : "unverified",
    last_source_reviewed_at: new Date().toISOString(),
    created_by: actor,
    updated_by: actor,
    metadata: {
