...
// Staff (admin/editor -- today, just the site's one admin account) post
// and reply as "Podium Watch" with the site's own brand logo, never
// their own real name or a personal avatar -- a real, explicit
// preference (the same reason "Coach Bell" as an email sign-off was
// wrong): interns shouldn't see the admin's personal identity. A plain
// writer's real name and their own uploaded avatar_url (install/61,
// nullable -- null renders as an initial-letter circle client-side) are
// unaffected, only ever shown for their own posts.
const STAFF_DISPLAY_NAME = "Podium Watch";
const STAFF_AVATAR_URL = "/images/branding/podium_watch_logo_dark.png";

async function authorDisplayInfo(ids) {
  const uniqueIds = [...new Set(ids.filter(Boolean))];
  const map = new Map();
  if (!uniqueIds.length) return map;

  const { data, error } = await supabaseAdmin.from("portal_profiles").select("id, full_name, role, avatar_url").in("id", uniqueIds);
  if (error) throw error;
  for (const row of data || []) {
    map.set(row.id, isStaffRole(row.role)
      ? { name: STAFF_DISPLAY_NAME, avatar_url: STAFF_AVATAR_URL }
      : { name: row.full_name || "A writer", avatar_url: row.avatar_url || null });
  }
  return map;
}

function withAuthorInfo(row, info) {
  const found = info.get(row.author_id);
  return { ...row, author_name: found?.name || "A writer", author_avatar_url: found?.avatar_url || null };
}
...
export async function listBoardPosts() {
  ...
  const info = await authorDisplayInfo([
  ...
  for (const reply of replies || []) {
    ...
    repliesByPost.get(reply.post_id).push(withAuthorInfo(reply, info));
  }

  return (posts || []).map((post) => ({
    ...withAuthorInfo(post, info),
...
export async function createBoardPost({ authorId, authorRole, body }) {
  ...
  const info = await authorDisplayInfo([authorId]);
  const author = info.get(authorId);
  await notifyStaffOfBoardActivity({ authorId, authorName: author?.name || "A writer", authorRole, kind: "post", body: cleanedBody });

  return { ...data, author_name: author?.name || "A writer", author_avatar_url: author?.avatar_url || null, replies: [] };
}
...
export async function createBoardReply({ postId, authorId, authorRole, body }) {
  ...
  const info = await authorDisplayInfo([authorId, post.author_id]);
  const author = info.get(authorId);
  await notifyStaffOfBoardActivity({
    authorId,
    authorName: author?.name || "A writer",
    authorRole,
    kind: "reply",
    body: cleanedBody,
    postAuthorName: info.get(post.author_id)?.name || "their"
  });

  return { ...data, author_name: author?.name || "A writer", author_avatar_url: author?.avatar_url || null };
}
