// Split Watch's database-backed service layer, matching the
...
import { nearestDistanceBucket, getStandardGoalsForAthletes, upsertStandardGoalFromRace } from "./athlete_goal_service.mjs";
import { findBlockingReadinessIssues } from "./race_readiness_service.mjs";
...
// race_sessions.scheduled_start_time is a plain SQL `time` column
// (install/11_RACE_COMMAND_CENTER.sql) -- a time-of-day only, paired with
// the session's own race_date to mean anything ("this race" already has
// its date; this is just what time of day it's expected to go off).
// Confirmed the hard way against real production: a full ISO timestamp
// string fails Postgres's `time` input parser outright (error 22007).
// Optional and clearable -- a coach who set one and changes their mind
// can blank the field again rather than being stuck with a stale time.
function cleanNullableTimeOfDay(value, label) {
  if (value === null || value === undefined || value === "") {
    return null;
  }
  const cleaned = cleanText(value, 20);
  if (!/^([01]\d|2[0-3]):([0-5]\d)(:([0-5]\d))?$/.test(cleaned)) {
    fail(`${label} must be a time in HH:MM format.`);
  }
...
// --- shared authorization helper for every api/split-watch/*.js ----
...
// includeArchived controls only whether an archived race is EXCLUDED
// (the default -- every working list: the race switcher, the main hub's
// "Your races" list, helper routing) or INCLUDED alongside active races
// (the hub's own "View archived races" panel passes this explicitly).
// There is no "archived only" mode here on purpose -- the hub page
// already knows which rows are archived from each row's own
// archived_at field and filters client-side, so one query shape serves
// both call sites without the caller needing to guess which archived
// races exist before asking for them.
export async function listSessions({ teamId, includeArchived = false }) {
  const team = await loadTeam(teamId);

  // Rehearsal Mode (install/25): a rehearsal never appears in the main
  // race list, race switcher, or helper race-selection page -- it's
  // practice data reachable only from its own source race's Plan page
  // (see lib/rehearsal_service.mjs's getRehearsalStatus()), never as a
  // peer entry alongside real races.
...
// A coach tidying up an old race list, not deleting anything -- every
// recorded time, result, and audit record stays exactly where it is.
// Archiving also turns spectator_visible off (an archived race must
// never keep sitting on some parent's day-switcher after the coach
// considers it done with) and is refused for a live race, matching the
// existing "can't delete a live race" precedent (a volunteer's device
// may be mid-sync against it).
export async function archiveSession({ teamId, sessionId }) {
  await loadTeam(teamId);
  const session = await loadSessionOrFail(teamId, sessionId);

  if (session.status === "live") {
    fail("A live race can't be archived. Finish it first.", 409);
  }

  const { data, error } = await supabaseAdmin
    .from("race_sessions")
    .update({ archived_at: new Date().toISOString(), spectator_visible: false })
    .eq("id", sessionId)
    .eq("team_id", teamId)
    .select("*")
    .single();
...
export async function unarchiveSession({ teamId, sessionId }) {
  await loadTeam(teamId);
...
export async function updateSessionDetails({ teamId, sessionId, body }) {
  ...
  // live -- never turned on implicitly. A rehearsal (install/25) can
  // never become spectator-visible, on purpose, and not even a coach can
  // override that -- this is what guarantees notifyRaceStatus() below
  // can never queue a real follower notification for practice data
  // (it already no-ops on spectator_visible=false, so keeping a
  // rehearsal permanently false is what protects that path, not a
  // separate is_rehearsal check inside notifyRaceStatus() itself).
  if (body.spectator_visible !== undefined) {
    const requestedVisible = cleanBoolean(body.spectator_visible);
    if (requestedVisible && session.is_rehearsal) {
      fail("A rehearsal cannot be made visible to parents or spectators.", 400);
    }
    updates.spectator_visible = requestedVisible;
  }
  // Optional -- shown on the parent link's pre-race waiting screen
  // (race day spec, Section 1/5) when a coach sets it during prep.
  // Purely informational: nothing about race timing itself reads this
  // column, so getting it wrong or leaving it blank has zero effect on
  // the actual race.
  if (body.scheduled_start_time !== undefined) {
    updates.scheduled_start_time = cleanNullableTimeOfDay(body.scheduled_start_time, "Scheduled race time");
  }
...
// Race day feedback (2026-08-25): the client used to call this, get a
// session back, and just assume ITS OWN just-captured wall-clock moment
// was now the race's canonical start -- even when the race was already
// live from an earlier call (another device, or this same device
// retrying). started_by_this_request tells the caller the truth: "false"
// means the race was already live and race_started_at below is
// whatever it already was, NOT this request's timestamp -- the caller
// must treat that as a join/recovery, never as its own fresh start. This
// is the fix for a real, confirmed bug: a device that believed it had
// started the race actually hadn't, and timed an entire race against its
// own wrong local anchor. See docs/DECISIONS.md, 2026-08-26.
export async function startRace({ teamId, sessionId, raceStartedAt, createdBy }) {
  const team = await loadTeam(teamId);
  const session = await loadSessionOrFail(teamId, sessionId);

  if (session.status === "live") {
    return { session, started_by_this_request: false };
  }
  if (session.status === "finished" || session.status === "reviewed") {
    fail("This race has already finished.", 409);
  }

  // Race Day Command Center (Project 2): the same blocking rules the
  // checklist displays are enforced here too, so the client can never be
  // told "ready" and then have the actual start rejected -- but only for
  // an official race. A rehearsal is deliberately exempt: Project 1's
  // whole purpose is letting a coach exercise the timing flow before the
  // roster/checkpoints are necessarily finished, and this project's own
  // instructions are not to redesign Rehearsal Mode without a confirmed
  // defect, which this isn't.
  if (!session.is_rehearsal) {
    const [{ data: checkpoints, error: cpError }, { count: participantCount, error: partError }] = await Promise.all([
      supabaseAdmin.from("race_checkpoints").select("id, is_finish").eq("race_session_id", sessionId),
      supabaseAdmin.from("race_participants").select("id", { count: "exact", head: true }).eq("race_session_id", sessionId)
    ]);
    if (cpError) throw cpError;
    if (partError) throw partError;

    const blockingIssues = findBlockingReadinessIssues({
      session,
    ...
    if (blockingIssues.length > 0) {
      fail(
        "This race isn't ready to start yet: " + blockingIssues.map((i) => i.explanation).join(" "),
        409,
        "not_ready"
      );
    }
  ...
  // Guarded on status still being non-live at write time, not just at
  // the read above -- two devices racing to start the same session
  // within the same instant must never both "win": whichever UPDATE
  // actually lands first is the only one that changes any rows, and the
  // loser's own update matches zero rows, so its own select().single()
  // throws and it can be told the truth (started_by_this_request: false)
  // rather than being handed back a false "you started it" success.
  const { data, error } = await supabaseAdmin
    .from("race_sessions")
  ...
  if (!data) {
    // Lost the race to start -- another request already flipped this
    // session to live between our read above and our guarded write.
    // Re-read the now-current (real) session so the caller still gets
    // back the true canonical race_started_at to recover from.
    const current = await loadSessionOrFail(teamId, sessionId);
    return { session: current, started_by_this_request: false };
  }

  await notifyRaceStatus(team, data, "live", createdBy);

  return { session: data, started_by_this_request: true };
}
...
// A false start (or an accidental early tap of Start Race) invalidates
// every split recorded so far -- they're all relative to the wrong start
// moment. Restarting clears them and puts the race back on the pre-race
// screen so the real start gets recorded cleanly, rather than trying to
// shift every existing split by some correction offset. Deliberately
// only allowed while status is "live" -- a finished/reviewed race has
// already gone through review and needs "duplicate" or manual correction
// instead, not a wholesale reset.
export async function restartRace({ teamId, sessionId }) {
  await loadTeam(teamId);
  const session = await loadSessionOrFail(teamId, sessionId);

  if (session.status !== "live") {
    fail("Only a race that is currently live can be restarted.", 409);
  }
  ...
  const { data, error } = await supabaseAdmin
    .from("race_sessions")
    .update({ status: "scheduled", race_started_at: null })
    .eq("id", sessionId)
    .eq("team_id", teamId)
    .select("*")
    .single();
...
// Pure math for Race Clock Adjustment -- unit tested directly (see
// scripts/test-split-watch.mjs). The coach tells us what the CORRECT
// official elapsed time is right now; we work backward to the signed
// correction that, subtracted from any split's raw elapsed time (itself
// always just wall_clock_captured_at - race_started_at, never altered),
// yields the corrected value. Storing one signed number rather than a
// new "adjusted start time" keeps every raw captured timestamp
// completely untouched -- this is purely a display/derivation-time
// correction, exactly what the race day spec calls for ("do not
// permanently alter raw capture timestamps").
export function computeRaceClockAdjustmentSeconds({ raceStartedAt, officialElapsedSeconds, nowMs = Date.now() }) {
  const startMs = Date.parse(raceStartedAt);
  if (Number.isNaN(startMs)) {
    fail("This race has no recorded start time yet.");
  }
  const rawElapsedNowSeconds = (nowMs - startMs) / 1000;
  return rawElapsedNowSeconds - officialElapsedSeconds;
}

// Coach-only clock correction (Section 4 of the race day spec). Applies
// the SAME signed delta to every already-captured split's elapsed_seconds
// (so a review of a race that was corrected mid-race is internally
// consistent -- one runner's Mile 1 and another's Finish both reflect
// the same corrected clock, not two different eras of "before/after
// the fix"). wall_clock_captured_at is never touched on any row -- the
// original raw capture instant is always recoverable by re-deriving
// from race_started_at + race_start_adjustment_seconds, which this
// function keeps as a running total, not a one-shot value, so a SECOND
// adjustment later in the same race still corrects every split
// (old and new) to the one current, single corrected timeline.
export async function adjustRaceClock({ teamId, sessionId, officialElapsedSeconds }) {
  await loadTeam(teamId);
  const session = await loadSessionOrFail(teamId, sessionId);

  if (session.status !== "live" && session.status !== "finished") {
    fail("Only a race that has started can have its clock adjusted.", 409);
  }
  if (!session.race_started_at) {
    fail("This race has no recorded start time yet.");
  }

  const cleanedOfficialSeconds = cleanNonNegativeNumberOrNull(officialElapsedSeconds, "Official elapsed time");
  if (cleanedOfficialSeconds === null) {
    fail("Enter the current official elapsed time.");
  }

  const newCorrectionSeconds = computeRaceClockAdjustmentSeconds({
    raceStartedAt: session.race_started_at,
    officialElapsedSeconds: cleanedOfficialSeconds
  });
  const previousCorrectionSeconds = Number(session.race_start_adjustment_seconds) || 0;
  const deltaSeconds = newCorrectionSeconds - previousCorrectionSeconds;

  // Every existing split shifts by the same delta -- read-modify-write
  // per row (not a single SQL "elapsed_seconds = elapsed_seconds - x"
  // update) because a DNS/DNF row's elapsed_seconds is genuinely null
  // and must stay null, never become a bare negative delta.
  const { data: existingSplits, error: splitsError } = await supabaseAdmin
    .from("race_splits")
  ...
  for (const split of existingSplits || []) {
    if (split.elapsed_seconds === null || split.elapsed_seconds === undefined) continue;
    const corrected = Math.max(0, Number(split.elapsed_seconds) - deltaSeconds);
    const { error: updateError } = await supabaseAdmin
      .from("race_splits")
      .update({ elapsed_seconds: corrected })
      .eq("id", split.id);
  ...
  const { data, error } = await supabaseAdmin
    .from("race_sessions")
    .update({
      race_start_adjustment_seconds: newCorrectionSeconds,
      race_clock_adjusted_at: new Date().toISOString(),
      race_clock_adjustment_note: `Set to ${cleanedOfficialSeconds.toFixed(1)}s official elapsed (${deltaSeconds >= 0 ? "-" : "+"}${Math.abs(deltaSeconds).toFixed(1)}s correction).`
    })
    .eq("id", sessionId)
    .eq("team_id", teamId)
    .select("*")
  ...
  return { session: data, correction_seconds: newCorrectionSeconds, delta_seconds: deltaSeconds, splits_recalculated: (existingSplits || []).length };
}
...
// Any status can be deleted (draft, scheduled, finished, reviewed,
// cancelled) except "live" -- a race actively being timed right now may
// have a volunteer's device mid-sync against it, and deleting out from
// under that would corrupt their in-progress session, not just discard
// data. Every child row (checkpoints, participants, goals, targets,
// splits, coach notes) cascades on delete (install/11_RACE_COMMAND_CENTER.sql),
// so this is a real, complete, single-statement removal, not a soft flag.
export async function deleteSession({ teamId, sessionId }) {
  await loadTeam(teamId);
  const session = await loadSessionOrFail(teamId, sessionId);

  if (session.status === "live") {
    fail("Finish or cancel this race before deleting it -- it may still be actively timed.", 409);
  }
...
// around line 340) is only ever "cross_country" or "track" -- Split
// Watch never distinguishes indoor/outdoor. team_roster_service.mjs's
...
// Split Watch has no way to know which one a coach means.
...
export async function listTeamRoster({ teamId, sport = "cross_country" }) {
  ...
  // grade lives on the season-specific roster entry, not on team_athletes
  // (team_athletes only carries the athlete's permanent identity fields) --
  // selected here so the client can group "Add all from roster" by grade
  // band (JH/HS) as well as gender, for a team that runs both levels off
  // one account.
  ...
  const gradeByAthleteId = new Map();
  for (const entry of entries || []) {
    if (entry.grade != null && !gradeByAthleteId.has(entry.athlete_id)) {
      gradeByAthleteId.set(entry.athlete_id, entry.grade);
    }
  }
  ...
  return {
    athletes: (athletes || [])
      ...
      .map((a) => ({ ...a, grade: gradeByAthleteId.get(a.id) ?? null }))
  };
}

// Without this, a coach re-adds the exact same runner to a new race every
// single week and has to retype every Goal A/B/C from scratch every time,
// even when nothing about that runner's goal actually changed. Only ever
// fills in a goal for a participant who has NONE yet (a brand-new
// race_participants row just inserted) -- it never touches or overwrites
// an existing goal, so this can't silently clobber something a coach
// already set for this race. Deliberately copies only Goal A/B/C
// (plain numbers, always safe) -- never pacing strategy or checkpoint
// targets, since those are tied to a specific set of checkpoints a new
// race may not share; copying them blind risks a stale, wrong-shaped
// Custom Pace plan the coach never asked for. Guest/manual runners
// (no team_athlete_id) are skipped -- there's no reliable way to match
// "the same guest" across races by name alone.
//
// Distance-aware -- this used to sort every prior race by date only,
// with no idea what distance any of them were, so a 2-mile goal could
// get silently carried forward as a "5K goal." Goal A now prefers the
// athlete's goal-book entry (lib/athlete_goal_service.mjs) for THIS
// race's distance bucket; Goal B/C (which the goal book doesn't track --
// it's one number per distance, not a full A/B/C set) still fall back to
// the most recent PRIOR race, but only one whose own distance maps to
// the SAME bucket as this race. A prior race at a different distance is
// never used for any slot, on purpose.
async function seedGoalsForNewParticipants({ teamId, sessionId, session, insertedRows }) {
  const athleteIds = [...new Set(insertedRows.filter((r) => r.team_athlete_id).map((r) => r.team_athlete_id))];
  if (athleteIds.length === 0) {
    return;
  }

  const targetBucket = nearestDistanceBucket(session.distance_meters);
  const newParticipantIdByAthleteId = new Map(
    insertedRows.filter((r) => r.team_athlete_id).map((r) => [r.team_athlete_id, r.id])
  );

  const standardGoalsByAthlete = await getStandardGoalsForAthletes({ teamId, athleteIds });

  const { data: priorParticipants, error: priorError } = await supabaseAdmin
    .from("race_participants")
    .select("id, team_athlete_id, race_sessions!inner(team_id, race_date, distance_meters)")
    .in("team_athlete_id", athleteIds)
    .neq("race_session_id", sessionId)
    .eq("race_sessions.team_id", teamId)
    .order("race_date", { foreignTable: "race_sessions", ascending: false });

  if (priorError) {
    throw priorError;
  }

  // Sorted most-recent-first, so the first SAME-DISTANCE row seen per
  // athlete is their most recent prior participation at this distance --
  // a prior race at any other distance is skipped entirely, never used
  // as a fallback for a different one.
  const mostRecentSameDistanceParticipantIdByAthlete = new Map();
  for (const row of priorParticipants || []) {
    if (mostRecentSameDistanceParticipantIdByAthlete.has(row.team_athlete_id)) {
      continue;
    }
    if (nearestDistanceBucket(row.race_sessions.distance_meters) === targetBucket) {
      mostRecentSameDistanceParticipantIdByAthlete.set(row.team_athlete_id, row.id);
    }
  }

  let priorGoalsByOldParticipantId = new Map();
  if (mostRecentSameDistanceParticipantIdByAthlete.size > 0) {
    const { data: priorGoals, error: goalsError } = await supabaseAdmin
      .from("race_goals")
      .select("race_participant_id, goal_slot, goal_seconds")
      .in("race_participant_id", [...mostRecentSameDistanceParticipantIdByAthlete.values()]);

    if (goalsError) {
      throw goalsError;
    }

    priorGoalsByOldParticipantId = new Map();
    for (const goal of priorGoals || []) {
      if (!priorGoalsByOldParticipantId.has(goal.race_participant_id)) {
        priorGoalsByOldParticipantId.set(goal.race_participant_id, {});
      }
      priorGoalsByOldParticipantId.get(goal.race_participant_id)[goal.goal_slot] = goal.goal_seconds;
    }
  }

  const newGoalRows = [];
  for (const [athleteId, newParticipantId] of newParticipantIdByAthleteId) {
    const oldParticipantId = mostRecentSameDistanceParticipantIdByAthlete.get(athleteId);
    const priorGoals = oldParticipantId ? (priorGoalsByOldParticipantId.get(oldParticipantId) || {}) : {};
    const standardGoalSeconds = standardGoalsByAthlete.get(athleteId)?.get(targetBucket);

    const goalASeconds = standardGoalSeconds ?? priorGoals.A;
    if (Number.isFinite(goalASeconds) && goalASeconds > 0) {
      newGoalRows.push({ race_participant_id: newParticipantId, goal_slot: "A", goal_seconds: goalASeconds });
    }
    if (Number.isFinite(priorGoals.B) && priorGoals.B > 0) {
      newGoalRows.push({ race_participant_id: newParticipantId, goal_slot: "B", goal_seconds: priorGoals.B });
    }
    if (Number.isFinite(priorGoals.C) && priorGoals.C > 0) {
      newGoalRows.push({ race_participant_id: newParticipantId, goal_slot: "C", goal_seconds: priorGoals.C });
    }
  }
  ...
  const { error: insertGoalsError } = await supabaseAdmin.from("race_goals").insert(newGoalRows);
  if (insertGoalsError) {
...
export async function saveParticipants({ teamId, sessionId, participants }) {
  ...
  if (toInsert.length) {
    const { data: insertedRows, error } = await supabaseAdmin
      ...
      .select("id, team_athlete_id");
    if (error) throw error;
    await seedGoalsForNewParticipants({ teamId, sessionId, session, insertedRows: insertedRows || [] });
  }
...
export async function saveGoals({ teamId, sessionId, participantId, goals }) {
  await loadTeam(teamId);
  const session = await loadSessionOrFail(teamId, sessionId);
  const participant = await assertParticipantInSession(sessionId, cleanUuid(participantId, "Participant"));
  ...
  // Quietly builds the athlete's goal book up from normal coaching
  // workflow instead of requiring a separate data-entry pass -- the
  // coach just set Goal A for a real (non-manual) runner, so that's
  // presumably their current, informed goal for this distance. Manual/
  // guest runners have no team_athlete_id and are skipped, same as
  // every other goal-book write path.
  const goalA = cleanedGoals.find((g) => g.goal_slot === "A");
  if (participant.team_athlete_id && goalA) {
    await upsertStandardGoalFromRace({
      teamAthleteId: participant.team_athlete_id,
      distanceMeters: session.distance_meters,
      goalSeconds: goalA.goal_seconds
    });
  }
...
// Shared by bulkApplyGoal() and saveGoalsBulk() below -- both compute
// Even Pace targets from a goal time, which would silently clobber a
// runner's carefully hand-entered Custom Pace checkpoint targets if
// applied to them blindly (saveTargets() has no idea a bulk caller
// shouldn't be touching them -- it just does what it's told). Anyone
// on custom_pace is reported as skipped instead of touched.
async function partitionParticipantsByStrategy(sessionId, participantIds) {
  const { data, error } = await supabaseAdmin
    .from("race_participants")
    .select("id, strategy, team_athlete_id")
  ...
  for (const id of participantIds) {
    ...
    if (!participant) {
      skipped.push({ participant_id: id, reason: "Not found in this race." });
    } else if (participant.strategy === "custom_pace") {
      skipped.push({ participant_id: id, reason: "Uses a custom pace plan -- edit their own card instead." });
    } else {
...
export async function bulkApplyGoal({ teamId, sessionId, participantIds, goalSlot, goalSeconds }) {
  ...
  const { eligible, skipped } = await partitionParticipantsByStrategy(sessionId, participantIds);
  ...
  for (const participant of eligible) {
    ...
    results.push({ participant_id: participant.id, ...result });
  }

  return { results, skipped };
}

// The bulk goals TABLE's save action -- unlike bulkApplyGoal() above (one
// shared goal time applied to a selected group), this is a different goal
// PER participant, matching whatever a coach typed into that runner's own
// row. Reuses saveTargets() per participant for the exact same reason
// bulkApplyGoal() does: it's the one place that keeps race_goals AND the
// even-pace race_targets in sync, so Live Race Mode's pacing math is
// never left stale after a bulk edit. Also quietly upserts the athlete
// goal book per participant, same as the single-runner saveGoals() path,
// so bulk edits build the goal book up too, not just individual ones.
export async function saveGoalsBulk({ teamId, sessionId, entries }) {
  ...
  const cleanedEntries = entries.map((e) => ({
    participantId: cleanUuid(e.participant_id, "Participant"),
    goalASeconds: cleanPositiveNumber(e.goal_a_seconds, "Goal time")
  }));
  const goalByParticipantId = new Map(cleanedEntries.map((e) => [e.participantId, e.goalASeconds]));
  const participantIds = cleanedEntries.map((e) => e.participantId);

  const { eligible, skipped } = await partitionParticipantsByStrategy(sessionId, participantIds);

  const results = [];
  for (const participant of eligible) {
    const goalASeconds = goalByParticipantId.get(participant.id);
    const result = await saveTargets({
      teamId,
      sessionId,
      participantId: participant.id,
...
export async function pushSplits({ teamId, sessionId, splits }) {
  ...
  for (const raw of splits) {
    ...
    if (error) {
      // A genuine race: two different devices each minted their OWN new
      // client_split_id for the SAME (participant, checkpoint) pair --
      // e.g. two volunteers covering the same checkpoint, or a coach's
      // device tapping alongside a volunteer's. The upsert above only
      // resolves conflicts on client_split_id, so this instead hits the
      // SEPARATE `unique (race_participant_id, race_checkpoint_id)`
      // constraint (install/11_RACE_COMMAND_CENTER.sql) as a raw Postgres
      // error. Found during the overnight audit: left unhandled, this
      // wasn't just a failure for this one split -- it threw immediately
      // out of the whole loop, discarding every OTHER split still queued
      // in the same sync batch, and since the losing device's local
      // record can never be marked synced (it can never win this
      // upsert), every future sync retried the same doomed split first
      // and failed the exact same way -- permanently blocking that
      // device from syncing anything else for the rest of the race.
      if (error.code === "23505" && String(error.message || "").includes("race_splits_race_participant_id_race_checkpoint_id_key")) {
        const { data: winningRow, error: fetchError } = await supabaseAdmin
          .from("race_splits")
          .select("*")
          .eq("race_participant_id", incoming.raceParticipantId)
          .eq("race_checkpoint_id", incoming.raceCheckpointId)
          .single();
        if (fetchError) throw fetchError;
        // Reported back under THIS device's own client_split_id so the
        // client can adopt the winning value as authoritative and stop
        // retrying a split that can never win -- see triggerSync() in
        // split-watch-live.js for the client-side half of this fix.
        results.push({ client_split_id: clientSplitId, split: winningRow, revision_written: false, conflicted: true });
        continue;
      }
      throw error;
    }
    ...
    // advances a runner to "started". Note this only records INTENT
    // here -- see below for why the actual write is a guarded, atomic
    // conditional update rather than trusting this function's own
    // in-memory participant snapshot.
    ...
    } else if (incoming.elapsedSeconds === null && checkpoint.is_finish && !statusUpdates.has(incoming.raceParticipantId)) {
      // Real bug found in a full audit (2026-08-27): undoing a mistaken
      // finish-checkpoint tap (Undo -- elapsed_seconds cleared to null)
      // previously left the runner's status stuck on "finished" forever,
      // with no recorded finish time, unless they happened to be tapped
      // again at the finish. That silently dropped them from the Review
      // page's "Copy team summary" text recap -- neither a finisher
      // (no finish_elapsed_seconds) nor listed as DNS/DNF (status was
      // never actually either of those). Reverting to "started" here
      // fixes that. Guarded below to only ever revert a runner CURRENTLY
      // "finished" -- never fights an explicit DNS/DNF call, and a real
      // later tap for this participant in the same batch already has
      // its own statusUpdates entry (the `!statusUpdates.has(...)` guard
      // here skips this branch for them).
      statusUpdates.set(incoming.raceParticipantId, "started_after_finish_undo");
    }
  }
  ...
  for (const [participantId, rawStatus] of statusUpdates) {
    const isFinishUndo = rawStatus === "started_after_finish_undo";
    const status = isFinishUndo ? "started" : rawStatus;
    let query = supabaseAdmin.from("race_participants").update({ status }).eq("id", participantId);

    if (isFinishUndo) {
      // Only reverts a runner CURRENTLY "finished" -- never touches an
      // explicit DNS/DNF, and never touches a runner some real split
      // elsewhere in this same batch already moved (that case never
      // reaches here at all, since it already has a different
      // statusUpdates entry -- see the `!statusUpdates.has(...)` guard
      // above).
...
// Race Day Health (build plan Project 5). Stamped on every real sync
// call (push_splits/pull_state -- see api/split-watch/sync.js) from ANY
// device, coach or helper, regardless of the race's own status: cheap,
// and evaluateRaceReadiness() (lib/race_readiness_service.mjs) is the
// one place that decides this is only worth SURFACING once a race is
// actually live. clockOffsetMs is optional -- only present once a
// device has completed at least one round trip (see split-watch-
// live.js's updateClockOffset()) -- and, when given, simply overwrites
// whatever was reported before: the LATEST device to sync wins, not an
// aggregate across every device (see install/29's own header comment
// for why that's a deliberate, honest simplification, not an oversight).
// Never allowed to fail the caller's real work -- a health signal is
// not worth losing a genuine split/pull over.
export async function recordSyncActivity({ sessionId, clockOffsetMs }) {
  const update = { last_sync_at: new Date().toISOString() };
  if (Number.isFinite(clockOffsetMs)) {
    update.last_reported_clock_offset_ms = clockOffsetMs;
    update.last_reported_clock_offset_at = new Date().toISOString();
  }
  const { error } = await supabaseAdmin.from("race_sessions").update(update).eq("id", sessionId);
  if (error) {
    console.error("recordSyncActivity failed (non-fatal):", error);
  }
}
...
export async function getTeamReview({ teamId, sessionId }) {
  ...
  // Found while wiring up Review-page polling (which needs session.status
  // to know how fast to poll and when to stop): this function previously
  // returned only { rows }, even though the client
  // (public/scripts/split-watch-review.js) has always read data.session
  // and data.team to render the race name, team name, race date, and to
  // populate the race switcher. detail.session/detail.team were already
  // loaded here (via pullState() -> getSessionDetail()) and simply never
  // passed through -- every one of those fields has been silently blank,
  // and the race switcher has never populated, on every real load of this
  // page until now.
  return { session: detail.session, team: detail.team, rows };
}
