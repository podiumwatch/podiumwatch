...
//
// One count("exact", head:true) query per week id, run in parallel,
// rather than one flat select("week_id") across every week reduced
// client-side -- found during the audit that followed a real
// vote-undercounting incident on Athlete/Team of the Week (2026-08-31,
// see docs/DECISIONS.md): an unbounded select silently caps at
// PostgREST's default 1,000-row limit with no error at all, so a flat
// select across enough weeks' worth of ballots would eventually
// undercount every week's total the exact same silent way once real
// ballot volume crossed that ceiling. count() is a real COUNT(*)
// aggregate with no such cap.
export async function getBallotCountsByWeek(weekIds) {
  ...
  cleanWeekIds.forEach((weekId, index) => {
    const result = countResults[index];
    if (result.error) {
      throw result.error;
    }
    counts.set(weekId, result.count || 0);
  });
