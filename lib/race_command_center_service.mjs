...
async function copyMostRecentGoals({ teamId, sessionId, insertedRows }) {
  ...
  const { data: priorParticipants, error: priorError } = await supabaseAdmin
    ...
    .select("id, team_athlete_id, race_sessions!inner(team_id, race_date)")
  ...
  // Sorted most-recent-first, so the first row seen per athlete is their
  // most recent prior participation -- keep only that one.
  const mostRecentParticipantIdByAthlete = new Map();
  for (const row of priorParticipants || []) {
    if (!mostRecentParticipantIdByAthlete.has(row.team_athlete_id)) {
      mostRecentParticipantIdByAthlete.set(row.team_athlete_id, row.id);
    }
  }
  if (mostRecentParticipantIdByAthlete.size === 0) {
    return;
  }
  ...
  const { data: priorGoals, error: goalsError } = await supabaseAdmin
    ...
    .in("race_participant_id", [...mostRecentParticipantIdByAthlete.values()]);

  if (goalsError) {
    throw goalsError;
  }
  if (!priorGoals || priorGoals.length === 0) {
    return;
  }

  const athleteIdByOldParticipantId = new Map(
    [...mostRecentParticipantIdByAthlete].map(([athleteId, participantId]) => [participantId, athleteId])
  );
  ...
  for (const goal of priorGoals) {
    const athleteId = athleteIdByOldParticipantId.get(goal.race_participant_id);
    const newParticipantId = athleteId ? newParticipantIdByAthleteId.get(athleteId) : null;
    if (!newParticipantId) {
      continue;
    }
    newGoalRows.push({
      race_participant_id: newParticipantId,
...
export async function saveParticipants({ teamId, sessionId, participants }) {
  ...
  if (toInsert.length) {
    ...
    if (error) throw error;
    await copyMostRecentGoals({ teamId, sessionId, insertedRows: insertedRows || [] });
  }
