...
//
// The code itself is DIFFERENT from those invite tokens in one
// deliberate way (real coach feedback, 2026-08-27): it's meant to be
// spoken aloud or written on a whiteboard and handed to several
// volunteers over the course of a whole meet, not shown to one person
// once. team_race_day_codes.code stores it in plaintext (alongside the
// unchanged code_hash every verification check actually reads) purely
// so getRaceDayCodeStatus() can hand it back to the *coach's own*
// dialog on demand -- adding a third helper mid-afternoon should never
// require regenerating (and thus breaking it for anyone who hasn't
// joined yet) just to read back the same code already given to the
// first two. Reasonable here specifically because of what this code
// protects: 4 digits, one team, rate-limited, instantly revocable, and
// already effectively "public" the moment it's posted for a timing
// crew -- unlike an athlete/guardian invite, which stays single-
// recipient, hash-only, reveal-once, unchanged.
//
...
export async function getRaceDayCodeStatus(teamId) {
  const { data, error } = await supabaseAdmin
    .from("team_race_day_codes")
    .select("team_id, active, created_at, last_used_at, expires_at, code")
    .eq("team_id", teamId)
    .maybeSingle();
  if (error) throw error;
  ...
  const active = data.active && !isExpired;
  // A code generated before install/28 ran has no stored plaintext --
  // never surface a stale/expired one either way, only ever the CURRENT
  // active code or nothing.
  return { ...data, active, code: active ? data.code || null : null };
}
...
// The raw code is stored (in `code`, alongside the unchanged `code_hash`
// every verification check actually reads) specifically so
// getRaceDayCodeStatus() can hand it back to the coach's own dialog
// again later -- see this file's header comment for why that's a
// deliberate, narrow exception to this codebase's usual hash-only
// pattern. Regenerating still isn't free, though: it's still the ONLY
// way to change what the code actually is, and doing so stops the OLD
// value from verifying for anyone who hasn't joined with it yet.
//
...
export async function regenerateRaceDayCode(teamId, userId, { revokeExistingHelpers = false } = {}) {
  ...
  const { error: upsertError } = await supabaseAdmin
    .from("team_race_day_codes")
    .upsert(
      { team_id: teamId, code_hash: codeHash, code: rawCode, active: true, created_by_user_id: userId, last_used_at: null, expires_at: expiresAt },
      { onConflict: "team_id" }
    );
...
export async function revokeRaceDayCode(teamId) {
  // Clears the stored plaintext too, not just `active` -- nothing left
  // that should ever be shown again once a coach has explicitly turned
  // access off.
  const { error } = await supabaseAdmin.from("team_race_day_codes").update({ active: false, code: null }).eq("team_id", teamId);
  if (error) throw error;
  const { error: deleteError } = await supabaseAdmin.from("race_day_sessions").delete().eq("team_id", teamId);
  if (deleteError) throw deleteError;
}
