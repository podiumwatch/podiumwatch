// Race Day Access: a second, lighter-weight way into Race Command
// Center alongside the existing full Podium Watch coach account
...
//
// requireRaceCommandCenterAccess() (the one export every
// api/race-command-center/*.js handler actually calls) is deliberately
...
// race-day code grants Race Command Center access and nothing else.
import crypto from "node:crypto";
import { supabaseAdmin } from "./supabase-admin.mjs";
import { requireTeamUser } from "./team_auth.mjs";
import { requireTeamMembership } from "./race_command_center_service.mjs";
...
// --- the one function every api/race-command-center/*.js handler calls -------
...
export async function requireRaceCommandCenterAccess(request, teamId) {
