...
import { supabaseAdmin, fetchInChunks } from "./supabase-admin.mjs";
import {
  cleanAthleteText,
...
const EVENT_DEFINITIONS = [
  ["xc_1_mile", "Cross Country 1 Mile", "cross_country", "time", "asc", ["1 mile xc", "1 mile cross country"]],
...
export async function loadLatestRankSnapshots(profileIds) {
  ...
  // Chunked the same way api/recruiting/index.js's lookups are -- this
  // took down the same production incident (2026-09-04) once profileIds
  // grew past what a single .in() query's URL can hold. See
  // fetchInChunks's own comment in lib/supabase-admin.mjs.
  const data = await fetchInChunks(
    "athlete_recruit_rating_rank_snapshots",
    "profile_id, event_group, state_class_rank, event_group_rank, captured_at",
    "profile_id",
    profileIds,
    (query, chunk) => query
