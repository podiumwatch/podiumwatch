...
export async function previewPerformanceImport(rows, defaults = {}, options = {}) {
  ...
  const schoolLookup = createProfiles ? await loadOhioSchoolLookup() : null;
  ...
  const previewRows = normalizedRows.map((row) => {
    ...
    if (!matches.length) {
      ...
      if (createProfiles && row.source_type === "official") {
        const school = schoolLookup.resolve(row.school_name);
        ...
        if (school) {
          return {
            ...
            resolved_school_name: school.school_name,
            matches: []
          };
        }
      }
...
async function createOfficialSourceProfile(row, { actor, batchId }) {
  ...
  const slug = slugifyAthlete(`${row.athlete_name} ${row.resolved_school_name} ${row.graduation_year}`);
...
const GRADE_TO_NUMBER = { FR: 9, SO: 10, JR: 11, SR: 12 };

// A fall cross country season is written as the calendar year it happens in
// (season_year 2025 for a Nov 2025 meet). A senior competing that fall
// graduates the following spring, so graduation year is season_year + 1,
// adjusted further back one year per grade below senior.
export function deriveGraduationYearFromGrade(grade, seasonYear) {
  const gradeNumber = GRADE_TO_NUMBER[cleanAthleteText(grade, 10).toUpperCase()];
  // Number(null) and Number("") are both 0, not NaN, so a missing season
  // year must be rejected before it reaches Number() -- otherwise a
  // missing value silently behaves like "season year zero" instead of
  // failing.
  const seasonProvided = seasonYear !== null && seasonYear !== undefined && seasonYear !== "";
  const season = seasonProvided ? Number(seasonYear) : NaN;

  if (!gradeNumber || !Number.isFinite(season)) {
    return null;
  }

  return season + 1 + (12 - gradeNumber);
}
...
export function parseOfficialResultsText(text, { season_year } = {}) {
  ...
  for (const line of lines) {
    ...
    rows.push({
      ...
      gender,
      graduation_year: deriveGraduationYearFromGrade(grade, season_year),
