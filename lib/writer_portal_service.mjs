...
export async function inviteWriter({ email, fullName, invitedBy }) {
  ...
  const { data, error } = await supabaseAdmin.auth.admin.inviteUserByEmail(cleanedEmail, {
    data: {
      portal_invite: true,
...
export async function resendAccountSetupLink({ email }) {
  ...
  const first = escapeHtml(String(existingUser.user_metadata?.full_name || "").trim().split(/\s+/)[0] || "there");
...
<p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#171717;">Here's a fresh link to set up (or reset) your Writer Portal password.</p>
...
<p style="margin:0 0 4px;font-size:15px;line-height:1.6;color:#171717;">Questions? Just reply to this email.</p>
  ...
  const text = `Hi ${existingUser.user_metadata?.full_name || "there"},\n\nHere's a fresh link to set up (or reset) your Writer Portal password.\n\nSet up your account: ${safeUrl}\n\nQuestions? Just reply to this email.`;
