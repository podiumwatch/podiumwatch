...
import {
  syncRosterAthleteProfile,
  syncTeamRosterProfiles,
...
// Graduation-year derivation now lives in athlete_foundation_service.mjs as
// deriveGraduationYearFromGrade(grade, seasonYear, sport) -- sport-aware
// (cross country vs. track resolve to different academic years for the
// same grade+season_year), shared across the roster, recruiting, and
// results-ingestion code paths instead of each computing it separately.
...
function normalizeImportRow(row, index, season) {
  ...
  if (!graduationYear && grade) {
    graduationYear = deriveGraduationYearFromGrade(grade, season.season_year, season.sport);
  }
...
async function saveAthlete(teamId, body, actor) {
  ...
  if (entryId) {
    // Deliberately NOT filtered by season_id here -- a coach can move an
    // existing entry onto a different (same-sport) season by submitting
    // a season_id that differs from the entry's current one, e.g.
    // splitting a mixed boys/girls roster apart. The entry is still
    // scoped to this team, so this can never touch another team's data.
    const { data, error } = await supabaseAdmin
      .from("team_roster_entries")
      .select("*")
      .eq("id", entryId)
    ...
    if (beforeEntry.season_id !== seasonId) {
      const { data: conflict, error: conflictError } = await supabaseAdmin
        .from("team_roster_entries")
        .select("id")
        .eq("season_id", seasonId)
        .eq("athlete_id", beforeEntry.athlete_id)
        .maybeSingle();
      ...
      if (conflict) {
        fail("This athlete is already on the selected season's roster.", 409);
      }
    }
  } else {
    const { data, error } = await supabaseAdmin
  ...
  if (beforeEntry) {
    ...
    if (error) {
      // Backstop for the same conflict the pre-check above already
      // guards against -- a concurrent request could still race past it.
      if (error.code === "23505") {
        fail("This athlete is already on the selected season's roster.", 409);
      }
  ...
  const moved = Boolean(beforeEntry) && beforeEntry.season_id !== seasonId;
  const changeVerb = moved ? "moved to" : (beforeEntry ? "updated on" : "added to");
  ...
  await writeTeamChange({
    ...
    summary: `${athlete.display_name || `${athlete.first_name} ${athlete.last_name}`} was ${changeVerb} ${season.name}.`,
...
async function commitImport(teamId, body, actor) {
  ...
  // team_commit_roster_import_v1 creates/updates team_athletes rows but was
  // never wired to the canonical athlete_profiles identity system --
  // syncRosterAthleteProfile() already runs correctly for a single-athlete
  // manual roster save, but a bulk import (almost certainly how most real
  // rosters, including Russia's, were first populated) never called it.
  // syncTeamRosterProfiles() is idempotent (it links only athletes that
  // don't already carry an athlete_profile_id) and internally swallows its
  // own errors, so this can never fail an otherwise-successful import --
  // it just closes the roster-to-profile gap going forward.
  const profileSync = await syncTeamRosterProfiles({
    teamId,
    actor: actor.label || actor.type || "Roster import"
  });
  if (profileSync?.error) {
    console.error("Roster profile sync after import failed for team:", teamId);
  }
...
export async function handleTeamRosterAction({
  ...
  if (action === "sync_profiles") {
    // A one-time, explicit, reviewable backfill for rosters that were
    // populated before commitImport() started calling this automatically
    // (see commit_import above) -- admin-only because it's a maintenance
    // action for an already-imported roster, not a normal coach task.
    if (actor.type !== "admin") {
      fail("Only Podium Watch admin can sync athlete profiles for a team.", 403);
    }

    const result = await syncTeamRosterProfiles({
      teamId: cleanTeamId,
      actor: actor.label || "Podium Watch Admin"
    });
    return {
      result,
      context: await loadRosterContext(cleanTeamId, cleanText(body.season_id, 100))
    };
  }
