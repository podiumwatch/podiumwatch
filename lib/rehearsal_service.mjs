...
export async function getRehearsalStatus({ teamId, sourceSessionId }) {
  ...
  // roster size, checkpoint count, finish checkpoint distance, or race
  // distance -- has changed since this rehearsal was created. Deliberately
  // narrow: a coach fixing a typo in a runner's name should not invalidate
  // a completed rehearsal, but adding or removing a runner or a
  // checkpoint, moving the finish, or changing the distance genuinely
  // could change what timing this race actually needs. Extended for
  // Race Day Command Center (Project 2), which surfaces this same
  // "outdated" flag in its readiness checklist and explicitly requires
  // checkpoint/finish-checkpoint changes to count, not just roster size.
  const [
    { count: currentParticipantCount },
    { count: rehearsalParticipantCount },
    { data: currentCheckpoints },
    { data: rehearsalCheckpoints }
  ] = await Promise.all([
    ...
    supabaseAdmin.from("race_checkpoints").select("id, is_finish, distance_meters").eq("race_session_id", source.id),
    supabaseAdmin.from("race_checkpoints").select("id, is_finish, distance_meters").eq("race_session_id", rehearsal.id)
  ]);

  const currentFinish = (currentCheckpoints || []).find((c) => c.is_finish);
  const rehearsalFinish = (rehearsalCheckpoints || []).find((c) => c.is_finish);

  const outdated =
    source.distance_meters !== rehearsal.distance_meters ||
    (currentParticipantCount || 0) !== (rehearsalParticipantCount || 0) ||
    (currentCheckpoints || []).length !== (rehearsalCheckpoints || []).length ||
    (currentFinish?.distance_meters ?? null) !== (rehearsalFinish?.distance_meters ?? null);
