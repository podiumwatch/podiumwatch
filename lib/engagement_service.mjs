...
const ANALYTICS_EVENT_TYPES = new Set([
  ...
  "story_view",
  // Same pattern as story_view, generalized to any standalone public
  // utility page that isn't a team page or an article -- content_id
  // carries the page's own path (public/scripts/page-view.js). First
  // used by /rankings/oatccc/.
  "page_view",
  // Podium Play (Phase 1) -- public/scripts/podium-play.js, the cooldown
  // arcade shown after a confirmed successful Athlete/Team of the Week
  // vote. content_type/content_id carry the game type where relevant;
  // team_id stays null, matching pace_calculator_use. Requires
  // install/40_PODIUM_PLAY_ANALYTICS.sql to actually be run before any of
  // these can insert -- see that file's header for why an allowlist entry
  // here alone is not sufficient.
...
// Site Presence: a real-time "who's here right now" companion to
// recordAnalyticsEvent's historical logging (see install/63_SITE_PRESENCE.sql
// for why this is a separate, overwrite-in-place table rather than another
// event type on team_analytics_events). Gated on the same analytics_enabled
// setting as page views, for the same reason -- a site operator who has
// turned off first-party analytics shouldn't have a heartbeat still firing
// quietly in the background.
async function recordPresencePing(body) {
  const settings = await getEngagementSettings();
  ...
  if (!sessionId) {
    const error = new Error("A session ID is required.");
    error.status = 400;
    throw error;
  }
  ...
  const { error } = await supabaseAdmin
    ...
    .upsert(
      {
        session_id: sessionId,
        visitor_id: cleanText(body.visitor_id, 100) || null,
...
// windowSeconds is deliberately a little more than presence.js's own
// heartbeat interval (see that file) so one missed beat from normal
// network jitter doesn't make an actually-still-there visitor blink out
// of the admin view. Also opportunistically sweeps rows old enough that
// no reasonable window would ever count them as active -- since every
// row is a real per-tab heartbeat, not an append-only log, there's no
// history value in keeping a session no one has pinged from in a day.
async function getLivePresence({ windowSeconds = 90 } = {}) {
  const staleCutoff = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  await supabaseAdmin.from("site_presence").delete().lt("updated_at", staleCutoff);

  const since = new Date(Date.now() - windowSeconds * 1000).toISOString();
  const { data, error } = await supabaseAdmin
    .from("site_presence")
    .select("path, visitor_id, updated_at")
...
function aggregateAnalytics(events) {
  ...
  // Same idea, for page_view: keyed by content_id, which for this event
  // type is the page's own path (e.g. "/rankings/oatccc/") rather than a
  // story slug -- kept as its own map, not merged into `stories`, so the
  // two never get mixed up in the admin UI.
  const pages = new Map();
  ...
  for (const event of events || []) {
    ...
    if (event.event_type === "page_view" && event.content_id) {
      pages.set(event.content_id, (pages.get(event.content_id) || 0) + 1);
    }
  }
