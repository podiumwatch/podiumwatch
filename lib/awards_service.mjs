...
// their schemas differ only in a handful of fields. As of 2026-09-14,
// AOTW is the one with boys/girls categories and two winners per week;
// TOTW combined its own boys/girls split away back on 2026-08-31 (see
// docs/DECISIONS.md) and stays single-winner.
...
// aotw_nominations.gender is real free text carried from two different
// sources (the public nomination form and the admin's own manual
// nomination form) that don't agree on casing ('Boys'/'Girls' vs.
// 'boys'/'girls', both confirmed live) -- normalized here rather than
// trusting either source, so aotw_finalists.category only ever gets the
// two real values its check constraint (install/59) allows. Returns
// null rather than guessing at anything else (a typo, a blank field) --
// a finalist with no category is a normal, pre-existing shape (every
// finalist from before this feature), not an error.
function normalizeAotwGender(value) {
  const normalized = cleanText(value, 30).toLowerCase();
  return normalized === "boys" || normalized === "girls" ? normalized : null;
}

// Team of the Week no longer distinguishes boys/girls (see the totw entry
// below), but totw_nominations.category / totw_finalists.category /
// totw_votes.category remain real, required columns on the existing
// hand-built schema -- every new row gets this fixed value instead of
// asking an admin or a public nominator to choose one.
...
const TYPES = {
  aotw: {
    ...
    // The real column holding a hashed voter identity -- confirmed live
    // directly against production: aotw_votes uses email_hash, while
    // totw_votes (below) uses voter_hash. Two different hand-built
    // schemas from before this project had a shared voting design; kept
    // as-is rather than migrated, since nothing requires them to match.
    voterColumn: "email_hash",
    // Real request, 2026-09-14: back to a separate boys' and girls'
    // Athlete of the Week each week (two winners), the exact reverse of
    // TOTW's own boys/girls -> combined migration below. Voting itself
    // is unchanged -- one shared, mixed-gender finalist pool and vote
    // count, same as today; category only matters at winner-selection
    // time (announceWinner requires at most one winner per category
    // once hasCategory is true, same rule TOTW used to follow before it
    // simplified away). aotw_finalists.category (install/59) is
    // nullable -- every finalist from before this feature has none, and
    // keeps working exactly as it already did as a single-winner week.
    hasCategory: true,
    // Vote tallying Redis cutover (2026-09-21): AOTW votes now live in
    // Redis, not aotw_votes -- see lib/award_vote_redis.mjs's own
    // header. Follows Team of the Week's own move (2026-09-15) onto the
    // same module; getWeekDetail() below branches on this flag exactly
    // as it already did for TOTW.
    voteBackend: "redis",
    ...
    finalistColumns: "id, week_id, category, athlete_name, school, grade, image_url, achievement, description, sort_order, winner, source_nomination_id",
    ...
    defaultsFromNomination(nomination) {
      ...
      return {
        identity_fields: { athlete_name: nomination.athlete_name, school: nomination.school, grade: nomination.grade, category: normalizeAotwGender(nomination.gender) },
  ...
  totw: {
    ...
    voterColumn: "voter_hash",
    // Team of the Week used to run separate boys/girls categories (two
    // winners per week). Too few nominations came in each week to fill
    // two categories, so this is now one combined Team of the Week --
    // hasCategory:false makes announceWinner() below require exactly one
    // winner, the same rule Athlete of the Week already follows.
    // totw_nominations.category / totw_finalists.category / totw_votes.category
    // are still real, NOT NULL columns on this pre-migration-system schema
    // (confirmed live, no default) -- rather than a migration to loosen
    // that constraint, every new row just gets the fixed TOTW_CATEGORY
    // value below, which also keeps voting cooldowns (grouped by
    // (voter_hash, category) in the existing cast_totw_vote RPC) correctly
    // shared across the whole combined list from now on, since every
    // finalist carries the same category. See docs/DECISIONS.md, 2026-08-31.
    hasCategory: false,
    // Vote tallying Redis pilot (2026-09-15): TOTW votes now live in
    // Redis, not totw_votes -- see lib/award_vote_redis.mjs's own
    // header. AOTW made the same move on 2026-09-21 (see its own
    // voteBackend entry above); getWeekDetail() below branches on this
    // flag for both.
    voteBackend: "redis",
    ...
    defaultsFromNomination(nomination) {
      return {
        identity_fields: { team_name: nomination.team_name, school: nomination.school, sport: nomination.sport, division: nomination.division, category: TOTW_CATEGORY },
        image_url: nomination.photo_url || "",
...
// How many real PEOPLE voted is a different question than "how many vote
// rows exist" (that's finalist.vote_count, a raw count). Both aotw_votes
// and totw_votes let the same voter identity cast more than one vote row
// across a week -- the public vote endpoints only enforce a 45-second
// cooldown, not a hard one-vote-per-person limit (confirmed live in real
// production data: the identical real email_hash appears twice, 46
// seconds apart, voting for the same finalist). Distinct voter identities
// is the honest "how many people voted" number.
//
// Fetched via bounded .range() pagination rather than one unbounded
// select -- the exact discipline the 2026-08-31 vote-undercounting
// incident (docs/DECISIONS.md) established: never trust a single page of
// results without a real signal that nothing was left behind. A single
// week's real vote volume today is in the hundreds, so this is normally
// one request; this stays correct even if that grows into the thousands.
const VOTER_HASH_PAGE_SIZE = 1000;

export function countDistinctVoters(hashValues) {
  return new Set((hashValues || []).filter(Boolean)).size;
}

async function fetchAllVoterHashesForWeek(votesTable, voterColumn, weekId) {
  const hashes = [];
  let offset = 0;
  ...
  for (;;) {
    const { data, error: fetchError } = await supabaseAdmin
      ...
      .select(voterColumn)
      .eq("week_id", weekId)
      .range(offset, offset + VOTER_HASH_PAGE_SIZE - 1);
    if (fetchError) throw fetchError;

    const rows = data || [];
    for (const row of rows) hashes.push(row[voterColumn]);

    if (rows.length < VOTER_HASH_PAGE_SIZE) break;
    offset += VOTER_HASH_PAGE_SIZE;
  }
...
export async function getWeekDetail({ type, weekId }) {
  ...
  if (cfg.voteBackend === "redis") {
    // Vote tallying Redis pilot (2026-09-15, extended to AOTW
    // 2026-09-21): see lib/award_vote_redis.mjs's own header. Dynamic
    // import, not a top-of-file one -- this module throws at import
    // time if the Redis env vars are missing (same fail-loud pattern as
    // supabase-admin.mjs), and this file is imported by
    // scripts/test-awards-service.mjs's own fake-Supabase-env test run,
    // which sets no Redis env at all and never calls getWeekDetail() --
    // a top-level import would break that test even though it's never
    // reached.
    const { getRedisVoteCounts, getRedisVoteStats } = await import("./award_vote_redis.mjs");
    const award = String(type || "").toLowerCase();
    const finalistRows = finalistsResult.data || [];
    const [voteCounts, stats] = await Promise.all([
      getRedisVoteCounts({ award, weekId: cleanWeekId, finalistIds: finalistRows.map((finalist) => finalist.id) }),
      getRedisVoteStats({ award, weekId: cleanWeekId })
    ]);
    finalists = finalistRows.map((finalist, index) => ({ ...finalist, vote_count: voteCounts[index] || 0 }));
    totalVotesCast = stats.totalVotesCast;
    distinctVoters = stats.distinctVoters;
  } else {
    // Real incident (2026-08-31): the public-facing api/aotw/current.js and
    // api/totw/current.js counted votes the same way this used to -- one
    // bulk select tallied client-side -- and Supabase/PostgREST silently
    // caps an unbounded select at 1,000 rows with no error at all. This
    // one used an explicit .limit(50000) rather than no limit, so it
    // hasn't broken yet at this site's real vote volume, but it is the
    // identical fragile shape (a hardcoded ceiling instead of a real
    // count) and would fail the exact same silent way once total votes
    // across every week ever (this query has no week_id to scope by)
    // passed 50,000. count:"exact", head:true is a genuine COUNT(*)
    // aggregate with no such cap, and stays correct at any real volume.
    const voteCountResults = await Promise.all(
      (finalistsResult.data || []).map((finalist) =>
        supabaseAdmin.from(cfg.votesTable).select("id", { count: "exact", head: true }).eq("finalist_id", finalist.id)
      )
    );
    for (const result of voteCountResults) if (result.error) throw result.error;

    finalists = (finalistsResult.data || []).map((finalist, index) => ({ ...finalist, vote_count: voteCountResults[index].count || 0 }));

    const voterHashes = await fetchAllVoterHashesForWeek(cfg.votesTable, cfg.voterColumn, cleanWeekId);
    totalVotesCast = voterHashes.length;
    distinctVoters = countDistinctVoters(voterHashes);
  }
...
// Auto-opens any week whose nomination_opens time has already passed but
// whose status is still stuck at "nominations_closed" -- called by
// api/cron/open-scheduled-nominations.js every 15 minutes. There is no
// other automated process anywhere in this codebase that flips this
// status on a schedule; without this, a week's nomination_opens timestamp
// can pass and the nomination form still never appears publicly, because
// api/aotw/current.js and api/totw/current.js gate visibility on `status`,
// not on comparing timestamps. Safe to call repeatedly -- a week already
// opened (or not yet due) simply won't match the query, so re-running
// this on every cron tick is a no-op for anything already handled.
export async function openScheduledNominations() {
  const opened = [];
  const failed = [];
  const nowIso = new Date().toISOString();

  for (const type of ["aotw", "totw"]) {
    const cfg = config(type);

    const { data: dueWeeks, error: dueError } = await supabaseAdmin
      .from(cfg.weeksTable)
      .select("id, week_slug, nomination_opens, status")
      .eq("status", "nominations_closed")
      .lte("nomination_opens", nowIso)
      .order("nomination_opens", { ascending: true });
    if (dueError) throw dueError;

    for (const week of dueWeeks || []) {
      try {
        const updated = await openNominations({ type, weekId: week.id });
        opened.push({ type, weekId: week.id, weekSlug: week.week_slug, status: updated.status });
      } catch (err) {
        failed.push({ type, weekId: week.id, weekSlug: week.week_slug, error: err?.message || String(err) });
      }
    }
  }
...
export function buildNominationInsert({ type, weekId, fields = {} }) {
  ...
  if (type === "totw") {
    ...
    return { ...base, category: TOTW_CATEGORY, team_name: teamName, school, sport, division: cleanText(fields.division, 50) || null, achievement };
  }
...
export async function updateFinalist({ type, finalistId, fields = {} }) {
  ...
  // AOTW only -- a safety valve for a finalist promoted with the wrong
  // category (a miscategorized nomination.gender, or one promoted before
  // this feature existed and never got one at all). TOTW's category
  // stays fixed to TOTW_CATEGORY; never editable through this action.
  if (type === "aotw" && fields.category !== undefined) update.category = normalizeAotwGender(fields.category);
  // A nomination's own team_name/athlete_name is often exactly what the
  // nominator typed ("Varsity boys" instead of the school's real name) --
  // this lets an admin clean that up after promotion without touching
  // anything else on the finalist. Real request, 2026-09-15.
  if (type === "totw" && fields.team_name !== undefined) update.team_name = cleanText(fields.team_name, 200);
  if (type === "aotw" && fields.athlete_name !== undefined) update.athlete_name = cleanText(fields.athlete_name, 200);
