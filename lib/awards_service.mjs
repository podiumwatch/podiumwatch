...
const TYPES = {
  ...
  totw: {
    ...
    // Redis, not totw_votes -- see lib/totw_vote_redis.mjs's own header.
    // AOTW has no voteBackend key at all and keeps reading aotw_votes
    // exactly as before; getWeekDetail() below branches on this flag.
    voteBackend: "redis",
...
export async function getWeekDetail({ type, weekId }) {
  ...
  if (cfg.voteBackend === "redis") {
    // Vote tallying Redis pilot (2026-09-15): see
    // lib/totw_vote_redis.mjs's own header. Dynamic import, not a
    // top-of-file one -- this module throws at import time if the Redis
    // env vars are missing (same fail-loud pattern as supabase-admin.mjs),
    // and getWeekDetail() is also called for AOTW, which must keep
    // working in any environment that never configured Redis at all
    // (including scripts/test-awards-service.mjs's own fake-Supabase-env
    // test run, which sets no Redis env at all).
    const { getRedisVoteCounts, getRedisVoteStats } = await import("./totw_vote_redis.mjs");
    const finalistRows = finalistsResult.data || [];
    const [voteCounts, stats] = await Promise.all([
      getRedisVoteCounts({ weekId: cleanWeekId, finalistIds: finalistRows.map((finalist) => finalist.id) }),
      getRedisVoteStats({ weekId: cleanWeekId })
    ]);
