...
export async function getWeekDetail({ type, weekId }) {
  ...
  const finalists = (finalistsResult.data || []).map((finalist, index) => ({ ...finalist, vote_count: voteCountResults[index].count || 0 }));
  ...
  return {
    ...
    total_votes_cast: voterHashes.length,
    distinct_voters: countDistinctVoters(voterHashes)
  };
}
