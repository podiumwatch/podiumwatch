...
import { deriveGraduationYearFromGrade, createOrLinkFinishTimingAthleteProfile } from "./athlete_foundation_service.mjs";
...
const ALLOWED_ROOTS = ["baumspage.com", "ohsaa.org", "milesplit.com", "finishtiming.com", "trackscoreboard.com", "timingfirst.com", "athletic.net", "runsignup.com", "racingliveresults.com", "milesplit.live", "seotiming.com", "pierretiming.com", "tfmeetpro.com", "directathletics.com", "championshiptiming.org", "track-scoreboard-default-rtdb.firebaseio.com"];
...
export async function storeSourceDocument(jobId, bytes, documentType, contentType) {
...
export async function createContentIngestionJob(input) {
  ...
  // meetName/meetDate/meetLocation/gender are optional context a caller can
  // supply directly (for example, a submitter typing them into a form)
  // rather than relying only on the pasted or uploaded content to state
  // them per row -- rowContract() already falls back to this context for
  // any row that does not carry its own value.
  const metadata = { sport: input.sport || null, seasonYear: input.seasonYear || null, meetName: input.meetName || null, meetDate: input.meetDate || null, meetLocation: input.meetLocation || null, gender: input.gender || null };
  const jobOptions = { dryRun: input.dryRun !== false, ...(input.extraOptions || {}) };
  const { data: job, error: jobError } = await supabaseAdmin.from("result_ingestion_jobs").insert({ job_type: input.jobType || "paste", provider_key: input.providerKey || "manual_upload", sport: input.sport || null, season_year: input.seasonYear || null, seeds: [{ file_name: safeName, meet_name: input.meetName || null, meet_date: input.meetDate || null }], options: jobOptions, parser_version: RESULTS_ENGINE_VERSION, created_by: input.createdBy || "Podium Watch Admin", status: "running", progress: { queued: 0, visited: 0, documents: 1, rows: 0, errors: 0 } }).select("*").single(); if (jobError) throw jobError;
  const extracted = type === "text" && input.jobType === "paste" ? { text: content, rows: parsePastedOrDelimitedText(content, metadata), warnings: [] } : await extractDocument({ bytes, text: ["html","text","csv","json"].includes(type) ? bytes.toString("utf8") : "", documentType: type, metadata });
...
const PUBLIC_SUBMISSION_DAILY_LIMIT = 8;
const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

// Public, unauthenticated submission path: anyone with a meet's raw results
// (a coach, a timer, a meet host) can paste or upload them without an
// admin account. This never bypasses the safety this project applies to
// every other import -- it is always a dry run landing in the same hidden
// review queue an admin already uses for crawled or uploaded content, and
// it is tagged so an approved row is later imported with community-level
// trust, not official-source trust (see commitPerformanceImport-adjacent
// handling in importApprovedRows).
export async function createPublicResultsSubmission(input) {
  // Honeypot: a field real submitters never see or fill in. A bot that
  // fills every field on the form will fill this one too. Report success
  // without doing any real work, so the bot gets no signal it was caught.
  if (clean(input.website, 200)) return { accepted: true, job_id: null, rows_found: 0 };

  const meetName = clean(input.meetName, 300);
  const meetDate = clean(input.meetDate, 30);
  const sport = clean(input.sport, 50);
  const seasonYear = Number(input.seasonYear);
  const submitterName = clean(input.submitterName, 200);
  const submitterEmail = clean(input.submitterEmail, 200).toLowerCase();
  const text = String(input.text || "");

  if (!meetName) throw error("Meet name is required.");
  if (!meetDate) throw error("Meet date is required.");
  if (!["cross_country", "indoor_track", "outdoor_track"].includes(sport)) throw error("Choose cross country, indoor track, or outdoor track.");
  if (!Number.isInteger(seasonYear) || seasonYear < 2000 || seasonYear > 2200) throw error("Season year is required.");
  if (!submitterName) throw error("Your name is required, so we can follow up with any questions about this meet.");
  if (!EMAIL_PATTERN.test(submitterEmail)) throw error("A valid email address is required, so we can follow up with any questions about this meet.");
  if (!text.trim()) throw error("Paste the results text, or choose a file to upload.");

  // The caller (the public API route) hashes the submitter's address
  // before this ever sees it, matching the same pattern already used by
  // the public athlete correction endpoint -- this rate limits without
  // retaining anyone's raw IP address.
  const ipHash = clean(input.ipHash, 100);
  if (ipHash) {
    const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const { count, error: countError } = await supabaseAdmin
      .from("result_ingestion_jobs")
      .select("id", { count: "exact", head: true })
      .eq("provider_key", "public_submission")
      .eq("options->>submitter_ip_hash", ipHash)
      .gte("created_at", since);
    if (countError) throw countError;
    if ((count || 0) >= PUBLIC_SUBMISSION_DAILY_LIMIT) {
      throw error("Too many submissions from this connection in the last day. Please try again tomorrow, or contact us directly if this is a mistake.", 429, "RATE_LIMITED");
    }
  }

  const job = await createContentIngestionJob({
    jobType: input.fileName ? "upload" : "paste",
    text,
    encoding: input.encoding,
    fileName: input.fileName,
    contentType: input.contentType,
    providerKey: "public_submission",
    sport,
    seasonYear,
    meetName,
    meetDate,
    meetLocation: clean(input.meetLocation, 300) || null,
    gender: clean(input.gender, 20) || null,
    createdBy: `Public submission: ${submitterName} <${submitterEmail}>`,
    dryRun: true,
    extraOptions: {
      is_public_submission: true,
      submitter_name: submitterName,
      submitter_email: submitterEmail,
      submitter_organization: clean(input.submitterOrganization, 200) || null,
      submission_note: clean(input.note, 2000) || null,
      submitter_ip_hash: ipHash || null,
      submitted_at: new Date().toISOString()
    }
  });

  return { accepted: true, job_id: job.id, rows_found: job.progress?.rows || 0 };
}
...
export async function runIngestionJob(jobId, sliceLimit = 10) {
  ...
  for (const page of queued || []) {
    ...
    try {
      ...
      if (type === "html" && page.depth < options.maxDepth) {
        // Discovering links is capped much more generously than maxPages
        // (which caps how many pages are actually fetched): a catalog page
        // can easily list dozens of sibling event pages at once, and if
        // discovery shared the same small cap as maxPages, that one page's
        // fan-out alone could exhaust it before any single event's own
        // linked result files (PDFs, etc.) were ever discovered -- those
        // children only become visible once their parent page is visited,
        // by which point there was no discovery capacity left for them.
        // Since real result files score far higher than plain navigation
        // pages (see scoreResultLink), the existing result_score-first
        // fetch order already prioritizes them the moment they exist in
        // the queue; this just makes sure they get the chance to.
        const maxQueuedPages = Math.min(2000, Math.max(options.maxPages * 10, 200));
        const { count: knownCount } = await supabaseAdmin.from("result_crawl_pages").select("id", { count: "exact", head: true }).eq("job_id", jobId); const capacity = Math.max(0, maxQueuedPages - Number(knownCount || 0)); const links = extractScoredLinks(body, loaded.finalUrl).filter((link) => link.score >= options.minLinkScore).slice(0, capacity); for (const link of links) { const next = { job_id: jobId, url: link.url, canonical_url: link.url, depth: page.depth + 1, provider_key: recognizeProvider(link.url), status: "queued", result_score: link.score, reason_codes: link.reasons, parent_page_id: page.id, source_chain: [...chain, link.url], page_context: pageContext }; const { data: nextPage } = await supabaseAdmin.from("result_crawl_pages").upsert(next, { onConflict: "job_id,canonical_url", ignoreDuplicates: true }).select("id").maybeSingle(); if (nextPage) await supabaseAdmin.from("result_crawl_edges").upsert({ job_id: jobId, from_page_id: page.id, to_page_id: nextPage.id, anchor_text: link.anchor, surrounding_text: link.surrounding, link_score: link.score, reason_codes: link.reasons }, { onConflict: "job_id,from_page_id,to_page_id", ignoreDuplicates: true }); } }
...
// Batched for the same reason resolveJobIdentities()'s lookups are
// below: PostgREST sends .in(column, values) as a URL query parameter,
// so reviewing a few hundred rows at once (a normal size for one large
// meet, not an edge case) blew the request's URL length limit outright
// ("Bad Request", no row count telling you why). Found 2026-08-31
// approving 706 rows in one real batch.
export async function reviewRows(jobId, rowIds, reviewStatus, note = "") {
  if (!['approved','rejected','pending'].includes(reviewStatus)) throw error("Invalid review status.");
  const update = { review_status: reviewStatus, review_note: clean(note, 2000), reviewed_at: reviewStatus === "pending" ? null : new Date().toISOString(), updated_at: new Date().toISOString() };
  const batches = await Promise.all(
    chunkValues(rowIds, IDENTITY_BATCH_SIZE).map((batchIds) => supabaseAdmin.from("result_staging_rows").update(update).eq("job_id", jobId).in("id", batchIds).select("id"))
  );
  let updatedCount = 0;
  for (const batch of batches) { if (batch.error) throw batch.error; updatedCount += batch.data?.length || 0; }
  await audit(jobId, "rows_reviewed", { status: reviewStatus, count: updatedCount });
  return { updated: updatedCount };
}

function normalizeIdentity(value) { return clean(value).toLowerCase().replace(/[^a-z0-9]+/g, " ").trim(); }

// Extracted from resolveJobIdentities so a provider-specific resolver (see
// lib/finish_timing_ingestion_service.mjs) can reuse the exact same "never
// merge on name alone" logic instead of a second copy of it. Pure -- takes
// pre-loaded lookup maps, does no fetching of its own.
export function matchSchoolCandidate(schoolNameNormalized, schoolMap) {
  const candidates = [...new Map((schoolMap.get(schoolNameNormalized) || []).map((item) => [item.id, item])).values()];
  return { school: candidates.length === 1 ? candidates[0] : null, candidates };
}

export function matchAthleteCandidate({ athleteName, gender, grade, seasonYear, sport, school, profileMap }) {
  const expectedGraduation = deriveGraduationYearFromGrade(grade, seasonYear, sport);
  const candidates = (profileMap.get(normalizeIdentity(athleteName)) || []).filter((profile) =>
    (!gender || !profile.gender || profile.gender === "unspecified" || profile.gender === gender) &&
    (!school || !profile.current_school_id || profile.current_school_id === school.id) &&
    (!expectedGraduation || !profile.graduation_year || profile.graduation_year === expectedGraduation)
  );
  return { candidates, matched: candidates.length === 1 ? candidates[0] : null };
}

// PostgREST sends .in(column, values) as a URL query parameter, so a
// large value list blows past the request's URL length limit and the
// whole query fails outright (the same real failure mode already fixed
// once in api/athletes/index.js and again in
// lib/athlete_leaders_service.mjs). resolveJobIdentities hit it for the
// first time 2026-08-31 on a real 959-row single-meet paste -- close to
// 900 distinct athlete names in one job is a normal size for a big
// invitational, not an edge case. Batches under that limit and merges
// the results back into one array.
const IDENTITY_BATCH_SIZE = 150;
...
async function selectInBatches(table, select, column, values) {
  if (!values.length) return [];

  const batches = await Promise.all(
    chunkValues(values, IDENTITY_BATCH_SIZE).map((batchValues) => supabaseAdmin.from(table).select(select).in(column, batchValues))
  );
...
export async function resolveJobIdentities(jobId) {
  const { data: rows, error: rowError } = await supabaseAdmin.from("result_staging_rows").select("id,athlete_name,school_name,gender,athlete_grade,season_year,sport,warning_codes").eq("job_id", jobId).in("review_status", ["pending","approved"]).limit(5000); if (rowError) throw rowError;
  ...
  const [profileRows, schoolRows, aliasRows] = await Promise.all([
    selectInBatches("athlete_profiles", "id,normalized_name,gender,graduation_year,current_school_id", "normalized_name", athleteNames),
    selectInBatches("ohio_schools", "id,normalized_name,school_name", "normalized_name", schoolNames),
    selectInBatches("ohio_school_aliases", "school_id,normalized_alias", "normalized_alias", schoolNames)
  ]);
  const schoolMap = new Map(); for (const school of schoolRows) schoolMap.set(school.normalized_name, [school]); for (const alias of aliasRows) schoolMap.set(alias.normalized_alias, [...(schoolMap.get(alias.normalized_alias) || []), { id: alias.school_id }]);
  const profileMap = new Map(); for (const profile of profileRows) profileMap.set(profile.normalized_name, [...(profileMap.get(profile.normalized_name) || []), profile]);
  let matched = 0, ambiguous = 0, unmatched = 0;
  for (const row of rows || []) {
    const { school, candidates: schoolCandidates } = matchSchoolCandidate(normalizeIdentity(row.school_name), schoolMap);
    const { candidates } = matchAthleteCandidate({ athleteName: row.athlete_name, gender: row.gender, grade: row.athlete_grade, seasonYear: row.season_year, sport: row.sport, school, profileMap });
...
// Purely additive -- resolveJobIdentities() above never creates a new
// athlete_profiles row on its own, for every provider this generic
// pipeline serves (Baumspage, MileSplit, public submissions, and now
// Athletic.net paste/upload): a real athlete simply not in the site
// yet is left in the exception queue with IDENTITY_UNMATCHED rather
// than guessed into existence. That's the right default for most
// sources, but leaves a real result permanently stuck for an athlete
// who legitimately just isn't in Podium Watch yet -- found 2026-08-31
// importing a real, large meet where the large majority of finishers
// had never been imported before. Called explicitly, only when asked
// for, never automatically from resolveJobIdentities itself.
//
// Only rows where the SCHOOL resolved confidently but the athlete did
// not are eligible -- a row with no confident school either has
// nothing safe to attach a new profile to. AMBIGUOUS_IDENTITY rows
// (more than one existing candidate) are also left alone: that is a
// real "which one" question for a human, not a "doesn't exist yet"
// case creating would ever fix. Reuses the exact same create-or-link/
// slug-retry logic the automatic Finish Timing pipeline already uses
// (see createOrLinkFinishTimingAthleteProfile), tagged with its own
// sourceTag so the two pipelines' created profiles are distinguishable
// later.
export async function createMissingAthleteProfiles(jobId, { publish = false, actor = "Podium Watch Admin" } = {}) {
  const { data: candidateRows, error: rowError } = await supabaseAdmin
    .from("result_staging_rows")
    .select("id,athlete_name,gender,athlete_grade,season_year,sport,matched_school_id,warning_codes")
    .eq("job_id", jobId)
    .is("matched_athlete_id", null)
    .not("matched_school_id", "is", null)
    .limit(5000);
  if (rowError) throw rowError;

  // warning_codes is a JSON column, not a native Postgres array --
  // PostgREST's .contains() filter errors on it ("invalid input syntax
  // for type json") rather than matching, so this is filtered in JS
  // instead of at the query.
  const rows = (candidateRows || []).filter((row) => (row.warning_codes || []).includes("IDENTITY_UNMATCHED"));

  let created = 0, linked = 0, skippedNoGrade = 0, errors = 0;

  for (const row of rows || []) {
    const expectedGraduation = deriveGraduationYearFromGrade(row.athlete_grade, row.season_year, row.sport);
    if (!expectedGraduation) { skippedNoGrade += 1; continue; }

    try {
      const result = await createOrLinkFinishTimingAthleteProfile({
        athleteName: row.athlete_name,
        gender: row.gender === "boys" || row.gender === "girls" ? row.gender : "unspecified",
        graduationYear: expectedGraduation,
        resolvedSchoolId: row.matched_school_id,
        publish,
        actor,
        sourceTag: "results-import"
      });

      if (result.created) created += 1; else linked += 1;

      const warningCodes = (row.warning_codes || []).filter((code) => code !== "IDENTITY_UNMATCHED");
      const { error: updateError } = await supabaseAdmin
        .from("result_staging_rows")
        .update({ matched_athlete_id: result.id, match_confidence: 100, warning_codes: warningCodes })
        .eq("id", row.id);
      if (updateError) throw updateError;
    } catch (createError) {
      errors += 1;
      console.error(`createMissingAthleteProfiles: could not create/link ${row.athlete_name}:`, createError.message || createError);
    }
  }

  await audit(jobId, "missing_profiles_created", { created, linked, skipped_no_grade: skippedNoGrade, errors, publish });
  return { created, linked, skippedNoGrade, errors };
}
...
export async function importApprovedRows(jobId) {
  ...
  // A row crawled or uploaded through the admin-controlled pipeline carries
  // the same institutional trust every other official import in this
  // project does. A public, unauthenticated submission (see
  // createPublicResultsSubmission) does not -- it is tagged community
  // trust instead, the same tier already used elsewhere in this project
  // for unverified, crowd-submitted recruiting activity.
  const isPublicSubmission = job.provider_key === "public_submission" || Boolean(job.options?.is_public_submission);
  const sourceType = isPublicSubmission ? "community" : "official";
  const sourceLabel = isPublicSubmission ? "Community submitted results" : "Official meet results";
  const verificationStatus = isPublicSubmission ? "unverified" : "source_linked";
  let imported = 0, duplicates = 0; const batchKey = `result-ingestion:${jobId}`;
  const { data: batch, error: batchError } = await supabaseAdmin.from("athlete_performance_import_batches").upsert({ import_key: batchKey, source_label: "Podium Watch Results Ingestion", source_type: sourceType, status: "started", requested_by: "Podium Watch Admin", options: { ingestion_job_id: jobId } }, { onConflict: "import_key" }).select("*").single(); if (batchError) throw batchError;
  for (const row of rows || []) {
...
// Purely additive -- does NOT touch importApprovedRows above, which
// Baumspage/other human-reviewed providers keep using exactly as before
// (ignoreDuplicates:true, so a later "correction" is silently dropped
// rather than applied -- fine for that path's independent third-party
// scrapes, where a changed mark already gets a brand-new source_key
// naturally). Finish Timing gives a genuinely stable per-athlete-per-event
// identity, so this function intentionally UPDATES an existing
// performance on a real change instead, and controls public_visible
// itself (the "publish" flag) rather than always defaulting to false.
export async function importFinishTimingApprovedRows(jobId, { publish = false } = {}) {
  const { data: job, error: jobError } = await supabaseAdmin.from("result_ingestion_jobs").select("*").eq("id", jobId).single();
  ...
  const { data: rows, error: rowError } = await supabaseAdmin
    ...
    .not("matched_athlete_id", "is", null)
    .limit(5000);
  if (rowError) throw rowError;

  // Group by the stable Finish Timing identity (one athlete in one event of
  // one meet) rather than by staging row -- a correction is, by this
  // engine's own design, a NEW staging row with a different
  // result_fingerprint (see result_parsers.mjs), so more than one staging
  // row can exist for the same real-world result. Only the newest
  // survives; older ones are marked "superseded" below (kept, never
  // deleted) instead of being imported a second time.
  //
  // A correction routinely arrives in a LATER scan than the original
  // import -- each calendar day is its own result_ingestion_jobs row
  // (findOrCreateDailyJob), so "today's approved row" and "the row that
  // already got imported" almost always live in two different jobs.
  // Comparing only within jobId would never see that older, already
  // -imported row, so it would never get marked superseded even though
  // a newer row for the same identity has since taken over. To catch
  // that, every candidate identity is re-checked against ALL staging
  // rows for its meet, across every job, not just this one.
  const finishTimingRows = (rows || []).filter((row) => {
    const raw = row.raw_row || {};
    return raw.providerMeetId && raw.providerEventId && raw.providerAthleteId;
  });

  const meetIds = [...new Set(finishTimingRows.map((row) => row.raw_row.providerMeetId))];
  const { data: crossJobRows, error: crossJobError } = meetIds.length
    ? await supabaseAdmin
        .from("result_staging_rows")
        .select("*")
        .in("raw_row->>providerMeetId", meetIds)
        .in("review_status", ["approved", "imported"])
        .limit(20000)
    : { data: [], error: null };
  if (crossJobError) throw crossJobError;

  const byIdentity = new Map();
  for (const row of [...finishTimingRows, ...(crossJobRows || [])]) {
    const raw = row.raw_row || {};
    if (!raw.providerMeetId || !raw.providerEventId || !raw.providerAthleteId) continue; // not a Finish Timing row
    const identityKey = `${raw.providerMeetId}|${raw.providerEventId}|${raw.providerAthleteId}`;
  ...
  // Only identities that actually have a newly-approved row THIS call are
  // reprocessed -- an identity with no new approved row today is left
  // completely untouched, even though it may appear in crossJobRows.
  ...
  const survivors = [...byIdentity.entries()]
    .filter(([identityKey]) => approvedIdentityKeys.has(identityKey))
    .map(([, row]) => row);
  const providerTeamIds = [...new Set(survivors.map((row) => row.raw_row?.providerTeamId).filter(Boolean))];
  const { data: teamLinks, error: teamLinkError } = providerTeamIds.length
    ? await supabaseAdmin.from("finish_timing_team_links").select("provider_team_id,matched_team_id").in("provider_team_id", providerTeamIds)
    : { data: [], error: null };
  if (teamLinkError) throw teamLinkError;
  const teamIdByProviderTeamId = new Map((teamLinks || []).map((link) => [link.provider_team_id, link.matched_team_id]));

  let published = 0, updated = 0, republished = 0, unchanged = 0, adminOverriddenSkipped = 0;

  for (const row of survivors) {
    const raw = row.raw_row || {};
    const sourceKey = `finish-timing:meet:${raw.providerMeetId}:event:${raw.providerEventId}:athlete:${raw.providerAthleteId}`;
    const shape = performanceShape(row);

    const { data: existing, error: existingError } = await supabaseAdmin
      .from("athlete_performances")
      .select("id, mark_text, place, public_visible, metadata")
      .eq("source_key", sourceKey)
      .maybeSingle();
    if (existingError) throw existingError;

    if (existing?.metadata?.admin_overridden === true) {
      adminOverriddenSkipped += 1;
      continue;
    }

    const performance = {
      profile_id: row.matched_athlete_id,
      school_id: row.matched_school_id,
      team_id: teamIdByProviderTeamId.get(raw.providerTeamId) || null,
      sport: row.sport || job.sport,
      season_year: row.season_year || job.season_year,
      event_name: row.event_name,
      event_key: row.event_code,
      mark_text: row.mark_text,
      mark_value: row.mark_value,
      mark_sort_value: row.mark_value,
      mark_unit: shape.mark_unit,
      measurement_type: shape.measurement_type,
      sort_direction: shape.sort_direction,
      record_type: "race_result",
      meet_name: row.meet_name,
      meet_date: row.meet_date,
      place: row.place,
      grade: Number(row.athlete_grade) || null,
      wind_text: row.wind_text,
      source_key: sourceKey,
      source_label: "Finish Timing",
      source_url: `https://finishtiming.trackscoreboard.com/meets/${raw.providerMeetId}/events/${raw.providerEventId}`,
      source_type: "official",
      verification_status: "source_linked",
      public_visible: publish,
      result_status: "official_result",
      notes: `Automatically imported from Finish Timing (meet ${raw.providerMeetId}, event ${raw.providerEventId}).`,
      metadata: { ingestion_job_id: jobId, staging_row_id: row.id, provider: "finish_timing", provider_meet_id: raw.providerMeetId, provider_event_id: raw.providerEventId, provider_athlete_id: raw.providerAthleteId }
    };

    if (!existing) {
      const { data: saved, error: saveError } = await supabaseAdmin.from("athlete_performances").upsert(performance, { onConflict: "source_key" }).select("id").single();
      if (saveError) throw saveError;
      published += 1;
      await supabaseAdmin.from("result_staging_rows").update({ review_status: "imported", imported_performance_id: saved.id, updated_at: new Date().toISOString() }).eq("id", row.id);
      await audit(jobId, "performance_published", { performance_id: saved.id, source_key: sourceKey });
      continue;
    }

    const changed = existing.mark_text !== performance.mark_text || existing.place !== performance.place;
    const visibilityChanged = existing.public_visible !== performance.public_visible;

    if (changed) {
      const { error: updateError } = await supabaseAdmin.from("athlete_performances").update(performance).eq("id", existing.id);
      if (updateError) throw updateError;
      updated += 1;
      await supabaseAdmin.from("result_staging_rows").update({ review_status: "imported", imported_performance_id: existing.id, updated_at: new Date().toISOString() }).eq("id", row.id);
      await audit(jobId, "performance_corrected", { performance_id: existing.id, source_key: sourceKey, old: { mark_text: existing.mark_text, place: existing.place }, new: { mark_text: performance.mark_text, place: performance.place } });
    } else if (visibilityChanged) {
      const { error: updateError } = await supabaseAdmin.from("athlete_performances").update({ public_visible: performance.public_visible }).eq("id", existing.id);
      if (updateError) throw updateError;
      republished += 1;
      await audit(jobId, "performance_republished", { performance_id: existing.id, source_key: sourceKey, public_visible: performance.public_visible });
    } else {
      unchanged += 1;
    }
  }

  // Every staging row that lost to a newer row for the same identity is
  // marked superseded so it never sits forever as an un-actioned
  // "approved" row (or a now-stale "imported" row from an earlier day's
  // job) and is never mistakenly imported a second time. Only identities
  // that actually got reprocessed this call are touched -- an identity
  // with no new approved row today keeps whatever status its rows
  // already have.
  const survivorIds = new Set(survivors.map((row) => row.id));
  const reprocessedIdentityKeys = new Set([...byIdentity.keys()].filter((key) => approvedIdentityKeys.has(key)));
  const supersededIds = [...new Set(
    [...finishTimingRows, ...(crossJobRows || [])]
      .filter((row) => {
        const raw = row.raw_row || {};
        const identityKey = `${raw.providerMeetId}|${raw.providerEventId}|${raw.providerAthleteId}`;
        return reprocessedIdentityKeys.has(identityKey) && !survivorIds.has(row.id);
      })
      .map((row) => row.id)
  )];
  if (supersededIds.length) {
    await supabaseAdmin.from("result_staging_rows").update({ review_status: "superseded", updated_at: new Date().toISOString() }).in("id", supersededIds);
  }

  return { published, updated, republished, unchanged, admin_overridden_skipped: adminOverriddenSkipped, superseded: supersededIds.length };
}
