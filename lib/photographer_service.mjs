...
export async function selfAddPortfolioItem(userId, photographerId, item) {
  ...
  if ((existing || []).length >= SELF_SERVICE_PORTFOLIO_LIMIT) {
    // A flat limit for now -- no plan-based limits exist yet (Phase
    // Four, unbuilt). Revisit once photographer_plans actually gates
    // anything.
    fail(`Portfolio images are limited to ${SELF_SERVICE_PORTFOLIO_LIMIT} for now.`, 409);
  }
