...
export async function adminSetSubscription(photographerId, { status, billing_interval, current_period_start, current_period_end, cancel_at_period_end, canceled_at, payment_status, admin_notes }) {
  ...
  const cleanedBillingInterval = billing_interval || null;
  const cleanedPeriodEnd = cleanDate(current_period_end, "renewal date");
  ...
  const willBeActive = status === "active" && (!cleanedPeriodEnd || new Date(cleanedPeriodEnd).getTime() > Date.now());
  const grantsStoryEligibility =
    willBeActive && cleanedBillingInterval === "annual" && (!existing || existing.partnership_story_status === "not_eligible");

  const updates = {
    photographer_id: cleanedId,
    status,
    billing_interval: cleanedBillingInterval,
