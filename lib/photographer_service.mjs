...
// Deliberately NOT a Stripe integration yet -- the database and
// entitlement architecture is real and functional; the payment processor
// itself is not connected. stripe_customer_id/stripe_subscription_id
// exist as real columns, already shaped for a future webhook handler, but
// nothing here ever writes a real Stripe id -- every row today is
// admin-entered manually (e.g. a photographer who paid outside of
// Stripe, or an admin activating membership ahead of checkout going
// live).
//
...
const SUBSCRIPTION_FIELDS =
  "id, photographer_id, status, billing_interval, stripe_customer_id, stripe_subscription_id, " +
  "current_period_start, current_period_end, cancel_at_period_end, canceled_at, payment_status, " +
  "partnership_story_status, admin_notes, updated_at";
...
export async function getSubscription(photographerId) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  return data || { status: "inactive", billing_interval: null, current_period_end: null, cancel_at_period_end: false, partnership_story_status: "not_eligible" };
}

// Admin-only: manually activate/update a photographer's subscription --
// the real, functional way to grant membership today, before checkout is
// wired up, and also the manual stand-in for what a future Stripe
// subscription-update webhook will do (e.g. a monthly -> annual switch).
// Never accepts a stripe_* id from the caller in a way that could be
// confused for a real webhook write -- every row this function touches is
// explicitly admin-entered.
//
// Partnership-story eligibility is granted here, exactly once, as a
// side effect of a subscription becoming genuinely active AND annual --
// never by the caller setting partnership_story_status directly, and
// never re-granted on a later save. Once partnership_story_status moves
// past 'not_eligible' it is never reset by this function, no matter how
// many times billing_interval or status changes afterward -- see
// install/17's design notes for why this is safe against repeated
// monthly<->annual switching.
export async function adminSetSubscription(photographerId, { status, billing_interval, current_period_start, current_period_end, cancel_at_period_end, canceled_at, payment_status, admin_notes }) {
  const cleanedId = cleanUuid(photographerId, "Photographer");
  if (!["inactive", "active", "trialing", "past_due", "canceled"].includes(status)) fail("Invalid subscription status.");
  if (billing_interval !== undefined && billing_interval !== "" && billing_interval !== null && !["monthly", "annual"].includes(billing_interval)) {
    fail("Invalid billing interval.");
  }
  if (payment_status !== undefined && payment_status !== "" && payment_status !== null && !["succeeded", "failed", "pending"].includes(payment_status)) {
  ...
  return upsertSubscriptionFields(cleanedId, {
    status,
    billing_interval: billing_interval || null,
    current_period_start: cleanDate(current_period_start, "period start date"),
    current_period_end: cleanDate(current_period_end, "renewal date"),
    cancel_at_period_end: cleanBoolean(cancel_at_period_end),
    canceled_at: cleanDate(canceled_at, "cancellation date"),
...
// (that's each caller's job; adminSetSubscription above cleans
...
// by BOTH the admin-manual path and the real Stripe webhook path so the
// "grant partnership-story eligibility exactly once, never on a later
// save" guarantee lives in exactly one place, never duplicated between
// them. `fields` may include any real photographer_subscriptions column
// -- a key simply left out of `fields` is left untouched by the upsert,
// which is how adminSetSubscription avoids ever touching stripe_customer_id/
// stripe_subscription_id (only the webhook path writes those) and how
// this function avoids ever touching partnership_story_status except
// when actually granting it.
export async function upsertSubscriptionFields(photographerId, fields) {
  ...
  const grantsStoryEligibility =
    willBeActive && fields.billing_interval === "annual" && (!existing || existing.partnership_story_status === "not_eligible");

  const updates = { photographer_id: photographerId, ...fields };
  if (grantsStoryEligibility) updates.partnership_story_status = "eligible";
...
// granted once (see adminSetSubscription above) and never reset on
