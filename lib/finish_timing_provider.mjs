...
export function parseAthleteRows(meetContext, eventId, eventDoc) {
  ...
  const eventName = isCrossCountry && Number.isFinite(Number(eventDoc.distance))
    ? `${eventDoc.distance} Mile`
    : "";
  ...
  for (const group of eventDoc.groups) {
    for (const athlete of group.athletes || []) {
      ...
      const rowDivision = String(athlete.division || athlete.divisionAthlete || "").toLowerCase();
      const eventLabel = String(eventDoc.name || eventDoc.fileNameOverride || "").toLowerCase();

      if (rowDivision && eventLabel && !rowDivision.includes(eventLabel) && !eventLabel.includes(rowDivision)) {
        warningCodes.push("INCONSISTENT_LEVEL_SIGNAL");
      }
