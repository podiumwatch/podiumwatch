...
import { loadAthleteViewRaces, loadSpectatorRace } from "./race_viewer_service.mjs";
...
// never another participant's plan. When the coach has separately
// turned a race's spectator_visible flag on, each race also carries a
// `leaderboard` field with the full spectator-safe view of every runner
// in that race, so a guardian can see how their athlete's teammates are
// doing without ever seeing their private goals/targets.
export async function getGuardianRaces(userId) {
  ...
  const races_with_leaderboard = await Promise.all(
    races.map(async (race) => {
      if (!race.session.spectator_visible) {
        return { ...race, leaderboard: null };
      }
      try {
        const leaderboard = await loadSpectatorRace(race.session.id);
        return { ...race, leaderboard };
      } catch {
        // Defensive only -- spectator_visible was just checked above, so
        // this should never actually throw NOT_VISIBLE. Never let a
        // leaderboard failure break the guardian's own race data.
        return { ...race, leaderboard: null };
      }
    })
  );

  return { races: races_with_leaderboard };
}
