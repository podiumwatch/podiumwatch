...
const RESULT_ROW_WITH_GRADE_PATTERN = /^(\d{1,4})\s+(.+?)\s+(FR|SO|JR|SR)\s+(.+?)\s+((?:(?:\d{1,2}:)?\d{1,2}\.\d{1,2})|(?:\d{1,3}[-'′]\d{1,2}(?:\.\d+)?)|(?:\d+(?:\.\d+)?m?)|DQ|DNF|DNS|SCR|NM|NT)(?:\s+[\d.]+)?\s*$/;
...
function textRows(text, initial = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (gradeMatch) { output.push({ place: gradeMatch[1], athlete_name: gradeMatch[2], athlete_grade: gradeMatch[3], school_name: gradeMatch[4], mark_text: gradeMatch[5], _row_number: index + 1, _context: { ...context }, rawText: line }); continue; }
