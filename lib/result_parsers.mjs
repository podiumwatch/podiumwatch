...
function textRows(text, initial = {}) {
  ...
  const output = []; let context = { ...initial }; let headers = null; let fixedColumns = null;
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    if (EVENT.test(line) && line.length < 150) context = { ...context, eventName: inferEvent(line, context.eventName), gender: inferGender(line, context.gender), competitionLevel: inferLevel(line, context.competitionLevel) };
...
async function pdfText(bytes) {
  ...
  for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
    ...
    for (const item of content.items) { const y = Math.round(item.transform?.[5] || 0); if (lastY !== null && Math.abs(y - lastY) > 2) { if (line.trim()) lines.push(line); line = ""; } line += `${item.str} `; lastY = y; }
