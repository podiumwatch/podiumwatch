...
// Bare distance tokens tolerate an optional trailing "m" (meters) with no
// space -- e.g. "Boys HS Open 5000m" -- so \b still lands after the "m"
// instead of failing to match at all because "0" and "m" share no word
// boundary. Found 2026-08-30 on a real GlenOak Eagle Invitational paste
// whose event headings all used this "5000m" shape; Bob Schul's own
// headings always had a space ("5000 Meters") so this never surfaced
// before, and every row under an unrecognized heading silently got no
// gender/competition_level/event at all (not merely a wrong guess).
const EVENT = /\b(5k(?:m)?|5000m?|3200m?|3000m?|1600m?|1500m?|800m?|400m?|300m?|200m?|100m?|60m?|hurdles?|relay|high jump|long jump|triple jump|pole vault|shot put|discus|hammer|javelin|cross country)\b/i;
const HEADER = /\b(place|pl|rank)\b.*\b(name|athlete|runner|team|school)\b.*\b(time|mark|result|performance)\b/i;
// OHSAA and MileSplit-style results pages print "place athlete grade team
// mark points" separated by single spaces, with the grade marker (FR/SO/JR/
// SR) embedded between the name and school rather than in its own aligned
// column -- there is no 2+ space run anywhere to split on, so neither the
// fixed-column nor the generic 2+-space-delimited paths below ever match a
// line like this. The grade marker itself is the only reliable anchor, and
// (as found and fixed in the admin recruiting importer's own copy of this
// same pattern) it must be matched case-sensitively: providers always
// render the real grade marker in capitals, but a mixed-case name suffix
// like "Jr" must not be mistaken for it. Found 2026-08-05 when a public
// results submission in this exact format silently produced zero rows.
//
// Athletic.net's own "Elite Performances" copy-paste export uses this
// exact same single-space-separated shape, but prints the raw grade
// number (9/10/11/12) in the "Yr" column instead of FR/SO/JR/SR -- found
// 2026-08-31 when a real Bob Schul Invitational paste (959 rows, all
// real, all correctly formatted otherwise) produced zero staged rows.
// The grade anchor now accepts either form; a leading space (also
// present in that real export, ahead of single-digit place numbers) is
// tolerated too rather than requiring the line to start exactly at the
// place number.
//
// The trailing points column tolerates "--" (a non-scorer marker) as
// well as a real number -- found 2026-08-30 on a real GlenOak Eagle
// Invitational paste where 292 of 669 rows (every non-scoring finisher
// in each Open heat) used "--" instead of a points value, which the
// bare [\d.]+ trailing group rejected outright, sending the whole line
// down to weaker fallback paths instead of just dropping the points.
const RESULT_ROW_WITH_GRADE_PATTERN = /^\s*(\d{1,4})\s+(.+?)\s+(FR|SO|JR|SR|1[0-2]|[6-9])\s+(.+?)\s+((?:(?:\d{1,2}:)?\d{1,2}\.\d{1,2})|(?:\d{1,3}[-'′]\d{1,2}(?:\.\d+)?)|(?:\d+(?:\.\d+)?m?)|DQ|DNF|DNS|SCR|NM|NT)(?:\s+(?:[\d.]+|-{1,2}))?\s*$/;
...
function eventCode(value, sport) {
  ...
  // Distance tokens tolerate an optional trailing "m" (meters) with no
  // space, same as EVENT above and for the same reason -- normalized()
  // keeps "5000m" as one alphanumeric run, so a plain \b5000\b never
  // matches it and every GlenOak-style "5000m" heading fell through to
  // the raw-text fallback event code instead of a real xc_5k/track_5000.
  const aliases = [
    [/\b5k\b|\b5000m?\b/, isCrossCountry ? "xc_5k" : "track_5000"],
    [/\b2 mile\b/, isCrossCountry ? "xc_2_mile" : "track_2_mile"],
    [/\b3200m?\b/, isCrossCountry ? "xc_3200" : "track_3200"],
    [/\b3k\b|\b3000m?\b/, isCrossCountry ? "xc_3k" : "track_3000"],
    ...
    [/\b1600m?\b/,"track_1600"],[/\b1500m?\b/,"track_1500"],[/\b800m?\b/,"track_800"],[/\b600m?\b/,"track_600"],[/\b500m?\b/,"track_500"],[/\b400m?\b/,"track_400"],[/\b300m?\b/,"track_300"],[/\b200m?\b/,"track_200"],[/\b100m?\b/,"track_100"],[/\b60m?\b/,"track_60"],
...
function textRows(text, initial = {}) {
  // Strips thousands-separator commas from distance-like numbers (e.g.
  // Athletic.net's "5,000 Meters" event header) before anything else
  // runs. EVENT below matches a bare "5000" token, not "5,000" -- with
  // the comma left in, an event heading is silently never recognized,
  // which cascades into every row under it missing eventName/gender/
  // competitionLevel entirely. Found 2026-08-31 alongside the numeric-
  // grade gap above, in the same real Athletic.net paste.
  const withoutThousandsCommas = String(text).replace(/(\d),(\d{3})\b/g, "$1$2");
  const lines = withoutThousandsCommas.replace(/\r/g, "").split("\n").map((line) => line.replace(/\u00a0/g, " ").replace(/\s+$/, "")).filter((line) => line.trim());
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // A data row's own place number, grade, or a track distance buried
    // in its time can accidentally satisfy EVENT too (place "100" reads
    // exactly like the "100" in "100 Meter Dash") -- excluding any line
    // that already matches the individual-result-row shape is what
    // keeps a real finisher from being mistaken for a new event heading
    // and silently overwriting the gender/event context for every row
    // after it. Found 2026-08-31 on a real 320-finisher Athletic.net
    // race, where place 100 onward all misread as an unrelated track
    // event once this ran unguarded.
    if (EVENT.test(line) && line.length < 150 && !RESULT_ROW_WITH_GRADE_PATTERN.test(line)) { inTeamScores = false; context = { ...context, eventName: inferEvent(line, context.eventName), gender: inferGender(line, context.gender), competitionLevel: inferLevel(line, context.competitionLevel) }; }
    ...
    // A header-shaped line that is actually single-space separated (for
    // example "PLACE VIDEO ATHLETE TEAM MARK POINTS") splits into exactly
    // one cell here, not real columns -- treating that as a one-column
    // header would make every data line beneath it match trivially and
    // produce a single garbage field instead of falling through to a
    // pattern that can actually parse it. Only accept genuinely
    // multi-column headers.
    if (HEADER.test(line)) { const headerCells = line.split(/\s{2,}|\t+/); if (headerCells.length > 1) headers = normalizedHeaders(headerCells); continue; }
    // Try the grade-anchored pattern before any fixed-column character-
    // position slicing below. It anchors on real row content (a grade
    // marker) rather than on where a keyword happened to sit in the
    // header line, so it isn't thrown off the way fixed-column slicing
    // already has a known real failure case for: a header like "Name
    // Year Team Finals Points" gives fixedColumns a column width derived
    // from THAT line's spacing, but single-space-delimited data rows
    // (e.g. "1 Tyler Brown 10 GlenOak 17:44.81 1") have variable-width
    // names/schools that almost never land on those same character
    // offsets, truncating names ("Tyler Brown" -> "Tyl") instead of
    // parsing them. Found 2026-08-30 on a real GlenOak Eagle
    // Invitational paste whose header used "Name"/"Team"/"Finals"
    // (unlike Bob Schul's "Athlete Yr Team Mark H#", which never
    // triggered fixedColumns at all) -- confirmed via isolated test that
    // this same widened grade pattern already parses those exact rows
    // correctly once it gets first look, before fixedColumns can.
    const gradeMatch = line.match(RESULT_ROW_WITH_GRADE_PATTERN);
    // A HY-TEK Team Scores row ("2 Willard  53  3 11 12 13 14 16 17") can
    // satisfy this same shape once the grade anchor also accepts a bare
    // number (see the comment above) -- one of its own trailing scoring
    // places lands right where a grade would. A real school name always
    // has a letter in it; a run of scoring places never does, so that is
    // the one reliable way to tell them apart here. Found 2026-08-31
    // testing the widened pattern against this project's own real
    // Baumspage fixture, which still has a live regression test for
    // exactly this leak (from 2026-08-05, before grades were widened).
    if (gradeMatch && /[A-Za-z]/.test(gradeMatch[4])) { output.push({ place: gradeMatch[1], athlete_name: gradeMatch[2], athlete_grade: gradeMatch[3], school_name: gradeMatch[4], mark_text: gradeMatch[5], _row_number: index + 1, _context: { ...context }, rawText: line }); continue; }
    if (fixedColumns) {
      ...
      if (/^\s*\d{1,4}\s+/.test(line) && line.length >= minimumWidth) {
        ...
        // Trailing points also tolerates "--" (a non-scorer marker), same
        // as RESULT_ROW_WITH_GRADE_PATTERN above and for the same reason.
        const resultTail = line.slice(fixedColumns.grade).match(/^\s*(?:(\d{1,2}|FR|SO|JR|SR)\s+)?(.+?)\s+((?:(?:\d{1,2}:)?\d{1,2}\.\d{1,3})|(?:\d{1,3}\s*[-'\u2032]\s*\d{1,2}(?:\.\d+)?)|(?:-?\d+(?:\.\d+)?(?:m|cm|ft|in)?)|DQ|DNF|DNS|SCR|NM|NT)(?:\s+(?:(\d+(?:\.\d+)?)|-{1,2}))?\s*$/i);
