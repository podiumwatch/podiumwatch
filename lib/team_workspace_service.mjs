...
import { getRaceDayContext, todayDateString } from "./todays_race_service.mjs";
...
async function getUpcomingRaceSessions(teamId, { limit = 5 } = {}) {
  ...
  // "upcoming race" -- it's practice data, not a peer entry.
...
export async function buildTeamHomeSummary({ teamId }) {
  ...
  // today's race, coach taps once." This is deliberately separate from
  // nextRace above -- nextRace is the soonest upcoming race on ANY future
  // date (useful for general season planning), while todaysRaceDay is
  // scoped tightly to right now: is anything live, is anything about to
  // run today, is anything already done today. See
  // lib/todays_race_service.mjs for the shared classification every
  // race-day-aware screen in Split Watch uses.
  const rawRaceDay = await getRaceDayContext(teamId);
  const liveRacesWithReadiness = await attachReadiness(rawRaceDay.liveRaces);
  const upcomingTodayWithReadiness = await attachReadiness(rawRaceDay.upcomingToday);
  const finishedTodayWithReadiness = await attachReadiness(rawRaceDay.finishedToday);
  // singleRelevantRace must point at the SAME (readiness-attached) object
  // identity as whichever array it came from -- re-resolved by id here
  // rather than reusing rawRaceDay's own reference, which still points
  // at the plain row from before attachReadiness() ran.
  const raceDaySessionById = new Map(
    [...liveRacesWithReadiness, ...upcomingTodayWithReadiness, ...finishedTodayWithReadiness].map((s) => [s.id, s])
  );
  const todaysRaceDay = {
    ...rawRaceDay,
    liveRaces: liveRacesWithReadiness,
    upcomingToday: upcomingTodayWithReadiness,
    finishedToday: finishedTodayWithReadiness,
    singleRelevantRace: rawRaceDay.singleRelevantRace ? raceDaySessionById.get(rawRaceDay.singleRelevantRace.id) : null
  };
