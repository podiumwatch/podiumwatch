...
export async function loadSpectatorRace(sessionId) {
  ...
  // Defense in depth: updateSessionDetails() already refuses to ever set
  // spectator_visible=true on a rehearsal (install/25), so this should
  // be unreachable -- checked explicitly anyway, since an official-only
  // boundary should never rely on a single upstream guard.
