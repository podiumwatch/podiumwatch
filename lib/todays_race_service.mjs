...
export function classifyRaceDay(sessions, today) {
  ...
  // > the next race today > the most recently finished race today.
  // Multiple simultaneously-live races is the one case that's a genuine
  // choice, never auto-resolved -- singleRelevantRace stays null and the
  // caller shows just those live races, nothing historical alongside them.
  ...
  } else if (liveRaces.length === 0 && upcomingToday.length === 0 && finishedToday.length > 0) {
