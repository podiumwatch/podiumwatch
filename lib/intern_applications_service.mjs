...
function buildInternWelcomeEmail({ fullName, actionLink, existingAccount }) {
  ...
  // Never put the raw one-time Supabase link directly in an email.
  // Confirmed live, real incident: Gmail's own automatic link-scanning
  // visited and consumed a real intern's invite token ~68 seconds after
  // the email sent, well before he ever clicked it himself -- so his
  // real click hit "Auth session missing!" (the token was already used).
  // Routing through /writer-login/?mode=confirm-invite instead means an
  // automated scanner just fetches a harmless static page; the sensitive
  // link only ever fires from a real click there.
  const ctaUrl = actionLink
    ? `https://podiumwatch.site/writer-login/?mode=confirm-invite&link=${encodeURIComponent(actionLink)}`
    : WRITER_LOGIN_URL;
