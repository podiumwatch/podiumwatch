...
// First-month-free trial (2026-09-24): applied to every new membership
// checkout, monthly or annual -- Stripe collects a payment method at
// checkout but does not charge it until the trial ends, and a
// photographer who cancels before then (through the Customer Portal,
// api/photographer/billing-portal.js) is never charged at all. One
// constant, read by both the real checkout session
// (lib/photographer_billing_service.mjs) and every page that describes
// it, so the trial length can never drift out of sync with the copy
// describing it.
